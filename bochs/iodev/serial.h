...
private:
  ...
  void lower_interrupt(Bit8u port);
  void raise_interrupt(Bit8u port, int type);

  void rx_fifo_enq(Bit8u port, Bit8u data);
  ...
  const char *ser_get_name(Bit32u address, Bit8u port, bool read);
