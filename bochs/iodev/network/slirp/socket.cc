...
{
    struct socket *so = (struct socket *)malloc(sizeof(struct socket));
    if (so) {
...
int sosendoob(struct socket *so)
{
    ...
    if (sb->sb_rptr < sb->sb_wptr) {
        ...
        n = (int)slirp_send(so, sb->sb_rptr, so->so_urgc,
                           ...
                           int flags)
{
     ...
     */
    assert(!((flags & SS_HOSTFWD) && (flags & SS_FACCEPTONCE)));
...
static bool sotranslate_out6(Slirp *s, struct socket *so, struct sockaddr_in6 *sin)
{
    ...
    if (in6_equal_net(&so->so_faddr6, &s->vprefix_addr6, s->vprefix_len) ||
        in6_equal(&so->so_faddr6, &ALLNODES_MULTICAST)) {
        if (s->disable_host_loopback) {
            return false;
        }
...
void sotranslate_accept(struct socket *so)
{
    ...
    case AF_UNIX: {
        ...
        if (so->slirp->in_enabled) {
            ...
            if (s < 0) {
                slirp_warning("Ephemeral slirp_socket() allocation failed", so->slirp->opaque);
                goto unix2inet_cont;
            }
            ...
            if (bind(s, (struct sockaddr *) &in_addr, sizeof(in_addr))) {
                slirp_warning("Ephemeral bind() failed", so->slirp->opaque);
                closesocket(s);
                goto unix2inet_cont;
            }
            in_addr_len = sizeof(in_addr);
            if (getsockname(s, (struct sockaddr *) &in_addr, &in_addr_len)) {
                slirp_warning("Ephemeral getsockname() failed", so->slirp->opaque);
                closesocket(s);
                goto unix2inet_cont;
            }
            so->s_aux = s;
