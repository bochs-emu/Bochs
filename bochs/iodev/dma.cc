...
bx_dma_c::read( Bit32u   address, unsigned io_len)
{
  ...
  switch (address) {
    ...
    case 0x008f: // DMA-2 page register, channel 4
      ...
      genlog->info("dma: read: unsupported address=%04x (channel %d)\n", 
    ...
    case 0x00dc:
    case 0x00de:
      genlog->info("dma: read: unsupported address=%04x\n", (unsigned) address);
      return(0x0000);
      break;
...
bx_dma_c::write(Bit32u   address, Bit32u   value, unsigned io_len)
{
  ...
  switch (address) {
    ...
    case 0x06:
      channel = address >> 1;
      if (bx_dbg.dma)
        genlog->info("  DMA-1 base and current address, channel %d\n", channel);
    ...
    case 0x07:
      channel = address >> 1;
      if (bx_dbg.dma)
        genlog->info("  DMA-1 base and current count, channel %d\n", channel);
      if (BX_DMA_THIS s.flip_flop==0) { /* 1st byte */
    ...
    case 0x08: /* DMA-1: command register */
      if (value != 0x04)
        genlog->info("DMA: write to 0008: value(%02xh) not 04h\n",
          (unsigned) value);
    ...
    case 0x09: // DMA-1: request register
      genlog->info("DMA-1: write to request register (%02x)\n", (unsigned) value);
      ...
      if (value & 0x04) {
        ...
        genlog->info("dma-1: cleared request bit for channel %u\n", (unsigned) channel);
        }
      return;
      break;
    ...
    case 0x0a:
      ...
      if (bx_dbg.dma)
        genlog->info("DMA1: set_mask_bit=%u, channel=%u, mask now=%02xh\n",
...
//genlog->info("DMA1: mode register[%u] = %02x\n",
//(unsigned) channel, (unsigned) value);
    ...
    case 0x0c: /* dma-1 clear byte flip/flop */
      if (bx_dbg.dma)
        genlog->info("DMA1: clear flip/flop\n");
    ...
    case 0x0d: // dma-1: master disable
      /* ??? */
      genlog->info("dma: master disable\n");
    ...
    case 0x0e: // dma-1: clear mask register
      genlog->info("dma-1: clear mask register\n");
    ...
    case 0x0f: // dma-1: write all mask bits
      genlog->info("dma-1: write all mask bits\n");
    ...
    case 0x87: /* dma page register, channel 0 */
      ...
      if (bx_dbg.dma)
        genlog->info("DMA1: page register %d = %02x\n", channel, (unsigned) value);
      return;
      break;
    ...
    //  if (value != 0x04)
    //    genlog->info("DMA2: write command register: value(%02xh)!=04h\n",
    //      (unsigned) value);
    //  return;
    //  break;
    ...
    case 0x00dc:
    case 0x00de:
      genlog->info("DMA(ignored): write: %04xh = %04xh\n",
        (unsigned) address, (unsigned) value);
      return;
      break;
    ...
    default:
      genlog->info("DMA(ignored): write: %04xh = %02xh\n",
        (unsigned) address, (unsigned) value);
    }
}
...
#endif
  if (!val) {
    //genlog->info("bx_dma_c::DRQ(): val == 0\n");
...
#if 0
  genlog->info("BX_DMA_THIS s.mask[2]: %02x\n", (unsigned) BX_DMA_THIS s.mask[2]);
  genlog->info("BX_DMA_THIS s.flip_flop: %u\n", (unsigned) BX_DMA_THIS s.flip_flop);
  genlog->info("BX_DMA_THIS s.status_reg: %02x\n", (unsigned) BX_DMA_THIS s.status_reg);
  genlog->info("mode_type: %02x\n", (unsigned) BX_DMA_THIS s.chan[channel].mode.mode_type);
  genlog->info("address_decrement: %02x\n", (unsigned) BX_DMA_THIS s.chan[channel].mode.address_decrement);
  genlog->info("autoinit_enable: %02x\n", (unsigned) BX_DMA_THIS s.chan[channel].mode.autoinit_enable);
  genlog->info("transfer_type: %02x\n", (unsigned) BX_DMA_THIS s.chan[channel].mode.transfer_type);
  genlog->info(".base_address: %04x\n", (unsigned) BX_DMA_THIS s.chan[channel].base_address);
  genlog->info(".current_address: %04x\n", (unsigned) BX_DMA_THIS s.chan[channel].current_address);
  genlog->info(".base_count: %04x\n", (unsigned) BX_DMA_THIS s.chan[channel].base_count);
  genlog->info(".current_count: %04x\n", (unsigned) BX_DMA_THIS s.chan[channel].current_count);
  genlog->info(".page_reg: %02x\n", (unsigned) BX_DMA_THIS s.chan[channel].page_reg);
#endif
...
genlog->info("dma_base = %08x\n", (unsigned) dma_base);
genlog->info("dma_base_count = %08x\n", (unsigned) BX_DMA_THIS s.chan[channel].base_count);
genlog->info("dma_roof = %08x\n", (unsigned) dma_roof);
    bx_panic("dma: DMA request outside 64k boundary\n");
    }


  //genlog->info("DRQ set up for single mode, increment, auto-init disabled, write\n");
...
bx_dma_c::raise_HLDA(bx_pc_system_c *pc_sys)
{
  ...
  //genlog->info("hlda: OK in response to DRQ(%u)\n", (unsigned) channel);
