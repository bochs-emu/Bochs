...
bool bx_usb_xhci_c::read_handler(bx_phy_address addr, unsigned len, void *data, void *param)
{
  ...
  // Register Port Sets
  else if ((offset >= XCHI_PORT_SET_OFFSET) && (offset < (XCHI_PORT_SET_OFFSET + (BX_XHCI_THIS hub.n_ports * 16)))) {
    unsigned port = (((offset - XCHI_PORT_SET_OFFSET) >> 4) & 0x3F); // calculate port number
