...
{
  if (!SIM->is_agp_device(BX_PLUGIN_RAGE128)) {
    BX_PANIC(("ATI Rage 128 PRO should be plugged into AGP slot"));
  }

  BX_RAGE128_THIS pci_enabled = true;

  BX_RAGE128_THIS put("RAGE128");
  // initialize SVGA stuffs.
  BX_RAGE128_THIS init_iohandlers(svga_read_handler, svga_write_handler, "rage128");
  DEV_register_ioread_handler(this, svga_read_handler, 0x03B4, "rage128", 2);
  DEV_register_ioread_handler(this, svga_read_handler, 0x03D0, "rage128", 6);
  DEV_register_ioread_handler(this, svga_read_handler, 0x03D2, "rage128", 2);
  DEV_register_ioread_handler(this, svga_read_handler, 0x03C3, "rage128", 3);
  DEV_register_iowrite_handler(this, svga_write_handler, 0x03D0, "rage128", 6);
  DEV_register_iowrite_handler(this, svga_write_handler, 0x03D2, "rage128", 2);
  // Register handlers for extended VGA ports (0xe000-0xe007, 0xe016-0xe017) to suppress log messages
  DEV_register_ioread_handler(this, svga_read_handler, 0xe000, "rage128", 8);
  DEV_register_iowrite_handler(this, svga_write_handler, 0xe000, "rage128", 8);
  DEV_register_ioread_handler(this, svga_read_handler, 0xe016, "rage128", 2);
  DEV_register_iowrite_handler(this, svga_write_handler, 0xe016, "rage128", 2);
  BX_RAGE128_THIS svga_init_members();
  BX_RAGE128_THIS svga_init_pcihandlers();
  BX_RAGE128_THIS s.CRTC.max_reg = RAGE128_CRTC_MAX;
...
#define SETUP_BITBLT(num, name, flags) \
  do { \
    BX_RAGE128_THIS rop_handler[num] = bitblt_rop_fwd_##name; \
    BX_RAGE128_THIS rop_flags[num] = flags; \
  } while (0);

void bx_rage128_c::bitblt_init()
{
...
void bx_rage128_c::execute_bitblt()
{
  ...
  Bit32u gmc = BX_RAGE128_THIS dp_gui_master_cntl;
  Bit32u dp_mix_val = BX_RAGE128_THIS dp_mix;
  ...
  Bit32u dst_off = BX_RAGE128_THIS dst_offset & 0x03FFFFFF;
  Bit32u dst_p = BX_RAGE128_THIS dst_pitch;
  ...
  Bit16s dst_x = (Bit16s)(BX_RAGE128_THIS dst_x & 0x3FFF);
  Bit16s dst_y = (Bit16s)(BX_RAGE128_THIS dst_y & 0x3FFF);
  Bit16u dst_w = (BX_RAGE128_THIS dst_width & 0x1FFF) + 1;
  Bit16u dst_h = (BX_RAGE128_THIS dst_height & 0x1FFF) + 1;
  
  // Extract direction from DP_CNTL
  Bit32u dp_cntl_val = BX_RAGE128_THIS dp_cntl;
  ...
  if (src_source == 2) {
    ...
    Bit32u src_off = BX_RAGE128_THIS src_offset & 0x03FFFFFF;
    Bit32u src_p = BX_RAGE128_THIS src_pitch;
    ...
    Bit16s src_x = (Bit16s)(BX_RAGE128_THIS src_x & 0x3FFF);
    Bit16s src_y = (Bit16s)(BX_RAGE128_THIS src_y & 0x3FFF);
    Bit32u src_addr = src_off + src_y * src_p + src_x * dst_bytes;
    
    // Perform screen-to-screen BitBlt
    Bit8u *dst_ptr = BX_RAGE128_THIS s.memory + (dst_addr & (BX_RAGE128_THIS s.memsize - 1));
    Bit8u *src_ptr = BX_RAGE128_THIS s.memory + (src_addr & (BX_RAGE128_THIS s.memsize - 1));
    ...
    if (x_dir && y_dir && BX_RAGE128_THIS rop_handler[rop3] != NULL && (BX_RAGE128_THIS rop_flags[rop3] & BX_ROP_PATTERN) == 0) {
      // Binary ROP (source and destination) - forward direction
      BX_RAGE128_THIS rop_handler[rop3](dst_ptr, src_ptr, dst_p, src_p, dst_w * dst_bytes, dst_h);
    } else {
    ...
    BX_RAGE128_THIS redraw_area(dst_x, dst_y, dst_w, dst_h);
    ...
    if (src_type == 0 || src_type == 1) {
      ...
      Bit32u fg_color = BX_RAGE128_THIS dp_src_frgd_clr;
      // Bit32u bg_color = BX_RAGE128_THIS dp_src_bkgd_clr; // TODO: Use for monochrome expansion
      ...
      Bit32u fill_color = BX_RAGE128_THIS dp_src_frgd_clr;
      for (unsigned y = 0; y < dst_h; y++) {
        for (unsigned x = 0; x < dst_w; x++) {
          if (dst_bytes == 1) {
  ...
  BX_RAGE128_THIS gui_eng_status = 0; // Operation complete
  BX_RAGE128_THIS gui_eng_ack = BX_RAGE128_THIS gui_eng_req; // Acknowledge request
}

void bx_rage128_c::svga_init_members()
{
  // clear all registers.
  BX_RAGE128_THIS crtc.index = RAGE128_CRTC_MAX + 1;
  for (int i = 0; i <= RAGE128_CRTC_MAX; i++)
    BX_RAGE128_THIS crtc.reg[i] = 0x00;

  // Initialize ATI Rage 128 registers
  memset(BX_RAGE128_THIS regs, 0, sizeof(BX_RAGE128_THIS regs));

  // Configuration registers
  BX_RAGE128_THIS config_cntl = 0;
  BX_RAGE128_THIS config_xstrap = 0;
  BX_RAGE128_THIS config_bonds = 0;
  BX_RAGE128_THIS config_memsize = 64 * 1024 * 1024; // 64MB default
  BX_RAGE128_THIS config_aper_0_base = 0;
  BX_RAGE128_THIS config_aper_1_base = 0;
  BX_RAGE128_THIS config_reg_1_base = 0;
  BX_RAGE128_THIS config_reg_aper_size = RAGE128_REG_APER_SIZE;
  BX_RAGE128_THIS config_aper_size = 0;

  // CRTC registers (ATI Rage 128 PRO addresses)
  // Main CRTC control registers
  BX_RAGE128_THIS crtc_gen_cntl = 0;
  BX_RAGE128_THIS crtc_ext_cntl = 0;
  BX_RAGE128_THIS crtc_status = 0;
  
  // Combined CRTC timing registers (ATI Rage 128 PRO format)
  BX_RAGE128_THIS crtc_h_total_disp = 0;
  BX_RAGE128_THIS crtc_h_sync_strt_wid = 0;
  BX_RAGE128_THIS crtc_v_total_disp = 0;
  BX_RAGE128_THIS crtc_v_sync_strt_wid = 0;
  
  // CRTC display control registers
  BX_RAGE128_THIS crtc_vline_crnt_vline = 0;
  BX_RAGE128_THIS crtc_crnt_frame = 0;
  BX_RAGE128_THIS crtc_gui_trig_vline = 0;
  BX_RAGE128_THIS crtc_debug = 0;
  BX_RAGE128_THIS crtc_offset = 0;
  BX_RAGE128_THIS crtc_offset_cntl = 0;
  BX_RAGE128_THIS crtc_pitch = 0;

  // Hardware cursor
  BX_RAGE128_THIS crtc_cur_offset = 0;
  BX_RAGE128_THIS crtc_cur_hv_pos = 0;
  BX_RAGE128_THIS crtc_cur_hv_offs = 0;
  BX_RAGE128_THIS crtc_cur_color0 = 0;
  BX_RAGE128_THIS crtc_cur_color1 = 0;

  // Clock control and PLL
  BX_RAGE128_THIS clk_cntl_index = 0;
  BX_RAGE128_THIS clk_cntl_data = 0;
  BX_RAGE128_THIS pll_ref_div = 0;
  BX_RAGE128_THIS pll_gen_cntl = 0;
  BX_RAGE128_THIS pll_vclk_cntl = 0;
  BX_RAGE128_THIS pll_vclk_cntl2 = 0;
  BX_RAGE128_THIS pll_vclk_cntl3 = 0;
  BX_RAGE128_THIS pll_vclk_ecp_cntl = 0;

  // DAC registers
  BX_RAGE128_THIS dac_cntl = 0;
  BX_RAGE128_THIS dac_ext_cntl = 0;
  BX_RAGE128_THIS dac_gen_cntl = 0;
  BX_RAGE128_THIS dac_macro_cntl = 0;
  BX_RAGE128_THIS dac_palette_addr = 0;
  BX_RAGE128_THIS dac_palette_data = 0;

  // GPIO registers for DDC communication
  BX_RAGE128_THIS gpio_vga_ddc = 0;
  BX_RAGE128_THIS gpio_dvi_ddc = 0;

  // GUI engine
  BX_RAGE128_THIS gui_eng_cntl = 0;
  BX_RAGE128_THIS gui_eng_status = 0;
  BX_RAGE128_THIS gui_eng_req = 0;
  BX_RAGE128_THIS gui_eng_ack = 0;

  // 2D Engine Destination Trajectory registers
  BX_RAGE128_THIS dst_offset = 0;
  BX_RAGE128_THIS dst_pitch = 0;
  BX_RAGE128_THIS dst_width = 0;
  BX_RAGE128_THIS dst_height = 0;
  BX_RAGE128_THIS src_x = 0;
  BX_RAGE128_THIS src_y = 0;
  BX_RAGE128_THIS dst_x = 0;
  BX_RAGE128_THIS dst_y = 0;
  BX_RAGE128_THIS src_pitch_offset = 0;
  BX_RAGE128_THIS dst_pitch_offset = 0;
  BX_RAGE128_THIS src_y_x = 0;
  BX_RAGE128_THIS dst_y_x = 0;

  // 2D Engine Source Trajectory registers
  BX_RAGE128_THIS src_offset = 0;
  BX_RAGE128_THIS src_pitch = 0;

  // 2D Engine Draw Engine Control registers
  BX_RAGE128_THIS dp_gui_master_cntl = 0;
  BX_RAGE128_THIS dp_cntl = 0;
  BX_RAGE128_THIS dp_datatype = 0;
  BX_RAGE128_THIS dp_mix = 0;
  BX_RAGE128_THIS dp_src_frgd_clr = 0;
  BX_RAGE128_THIS dp_src_bkgd_clr = 0;
  memset(BX_RAGE128_THIS host_data, 0, sizeof(BX_RAGE128_THIS host_data));
  BX_RAGE128_THIS host_data_last = 0;

  // Interrupts
  BX_RAGE128_THIS crt_intr = 0;
  BX_RAGE128_THIS crt_intr_en = 0;

  BX_RAGE128_THIS svga_unlock_special = 0;
  BX_RAGE128_THIS svga_needs_update_tile = 1;
  BX_RAGE128_THIS svga_needs_update_dispentire = 1;
  BX_RAGE128_THIS svga_needs_update_mode = 0;
  BX_RAGE128_THIS svga_double_width = 0;

  BX_RAGE128_THIS svga_xres = 640;
  BX_RAGE128_THIS svga_yres = 480;
  BX_RAGE128_THIS svga_bpp = 8;
  BX_RAGE128_THIS svga_pitch = 640;
  BX_RAGE128_THIS bank_base[0] = 0;
  BX_RAGE128_THIS bank_base[1] = 0;

  BX_RAGE128_THIS hw_cursor.x = 0;
  BX_RAGE128_THIS hw_cursor.y = 0;
  BX_RAGE128_THIS hw_cursor.size = 0;
  BX_RAGE128_THIS hw_cursor.offset = 0;
  BX_RAGE128_THIS hw_cursor.bpp32 = false;
  BX_RAGE128_THIS hw_cursor.enabled = false;

  // ATI Rage 128 PRO defaults to 32MB
  BX_RAGE128_THIS s.memsize = 32 * 1024 * 1024;
  BX_RAGE128_THIS config_memsize = BX_RAGE128_THIS s.memsize;

  // memory allocation.
  if (BX_RAGE128_THIS s.memory == NULL)
    BX_RAGE128_THIS s.memory = new Bit8u[BX_RAGE128_THIS s.memsize];
  memset(BX_RAGE128_THIS s.memory, 0x00, BX_RAGE128_THIS s.memsize);
  BX_RAGE128_THIS disp_ptr = BX_RAGE128_THIS s.memory;

  // VCLK defaults after reset
  BX_RAGE128_THIS s.vclk[0] = 25180000;
  BX_RAGE128_THIS s.vclk[1] = 28325000;
  BX_RAGE128_THIS s.vclk[2] = 41165000;
  BX_RAGE128_THIS s.vclk[3] = 36082000;
}
...
void bx_rage128_c::reset(unsigned type)
{
  ...
  // reset SVGA stuffs.
  BX_RAGE128_THIS svga_init_members();
  BX_RAGE128_THIS ddc.init();
  // Disable ROM shadowing to allow clearing of VRAM
  BX_RAGE128_THIS pci_conf[0x50] = 0x00;
}
...
void bx_rage128_c::register_state(void)
{
  ...
  new bx_shadow_num_c(crtc, "index", &BX_RAGE128_THIS crtc.index, BASE_HEX);
  new bx_shadow_data_c(crtc, "reg", BX_RAGE128_THIS crtc.reg, RAGE128_CRTC_MAX + 1, 1);
  BXRS_PARAM_BOOL(list, svga_unlock_special, BX_RAGE128_THIS svga_unlock_special);
  BXRS_PARAM_BOOL(list, svga_double_width, BX_RAGE128_THIS svga_double_width);
  new bx_shadow_num_c(list, "svga_xres", &BX_RAGE128_THIS svga_xres);
  new bx_shadow_num_c(list, "svga_yres", &BX_RAGE128_THIS svga_yres);
  new bx_shadow_num_c(list, "svga_pitch", &BX_RAGE128_THIS svga_pitch);
  new bx_shadow_num_c(list, "svga_bpp", &BX_RAGE128_THIS svga_bpp);
  new bx_shadow_num_c(list, "svga_dispbpp", &BX_RAGE128_THIS svga_dispbpp);
  new bx_shadow_num_c(list, "bank_base0", &BX_RAGE128_THIS bank_base[0], BASE_HEX);
  new bx_shadow_num_c(list, "bank_base1", &BX_RAGE128_THIS bank_base[1], BASE_HEX);
  bx_list_c *cursor = new bx_list_c(list, "hw_cursor");
  new bx_shadow_num_c(cursor, "x", &BX_RAGE128_THIS hw_cursor.x, BASE_HEX);
  new bx_shadow_num_c(cursor, "y", &BX_RAGE128_THIS hw_cursor.y, BASE_HEX);
  new bx_shadow_num_c(cursor, "size", &BX_RAGE128_THIS hw_cursor.size, BASE_HEX);
  register_pci_state(list);
}

void bx_rage128_c::after_restore_state(void)
{
  bx_pci_device_c::after_restore_pci_state(rage128_mem_read_handler);
  BX_RAGE128_THIS bx_vgacore_c::after_restore_state();
}

void bx_rage128_c::redraw_area(unsigned x0, unsigned y0,
                               unsigned width, unsigned height)
{
  redraw_area((Bit32s)x0, (Bit32s)y0, width, height);
}

void bx_rage128_c::redraw_area(Bit32u offset, Bit32u width, Bit32u height)
{
  if (BX_RAGE128_THIS svga_pitch != 0) {
    Bit32u redraw_x = offset % BX_RAGE128_THIS svga_pitch / (BX_RAGE128_THIS svga_bpp >> 3);
    Bit32u redraw_y = offset / BX_RAGE128_THIS svga_pitch;
    BX_RAGE128_THIS redraw_area(redraw_x, redraw_y, width, height);
  }
}
...
void bx_rage128_c::redraw_area(Bit32s x0, Bit32s y0, Bit32u width, Bit32u height)
{
  ...
  if (!(BX_RAGE128_THIS crtc_gen_cntl & (1 << 24))) {
    BX_RAGE128_THIS bx_vgacore_c::redraw_area(x0, y0, width, height);
    return;
  }

  if (BX_RAGE128_THIS svga_needs_update_mode) {
    return;
  }
  ...
  if (x0 < (Bit32s)BX_RAGE128_THIS svga_xres) {
    xt1 = (x0 + width - 1) / X_TILESIZE;
  } else {
    xt1 = (BX_RAGE128_THIS svga_xres - 1) / X_TILESIZE;
  }
  if (y0 < (Bit32s)BX_RAGE128_THIS svga_yres) {
    yt1 = (y0 + height - 1) / Y_TILESIZE;
  } else {
    yt1 = (BX_RAGE128_THIS svga_yres - 1) / Y_TILESIZE;
  }
  if ((x0 + width) > BX_RAGE128_THIS svga_xres) {
    BX_RAGE128_THIS redraw_area(0, y0 + 1, x0 + width - BX_RAGE128_THIS svga_xres, height);
  }
...
void bx_rage128_c::vertical_timer()
{
  bx_vgacore_c::vertical_timer();
  if (BX_RAGE128_THIS vtimer_toggle) {
    BX_RAGE128_THIS crt_intr |= 0x00000001;
    BX_DEBUG(("vertical_timer, crt_intr_en = %d",
      BX_RAGE128_THIS crt_intr_en));
    set_irq_level(true);
  }
}

bool bx_rage128_c::rage128_mem_read_handler(bx_phy_address addr, unsigned len,
                                        void *data, void *param)
{
  // BAR0: Linear Frame Buffer (LFB) - VRAM access
  if (addr >= BX_RAGE128_THIS pci_bar[0].addr &&
      addr < (BX_RAGE128_THIS pci_bar[0].addr + (64 * 1024 * 1024))) {
    Bit32u offset = (addr - BX_RAGE128_THIS pci_bar[0].addr) & ((64 * 1024 * 1024) - 1);
    Bit8u *data_ptr = (Bit8u *) data;
    for (unsigned i = 0; i < len; i++) {
      data_ptr[i] = BX_RAGE128_THIS s.memory[offset + i];
    }
    return 1;
  }

  // BAR2: Register aperture (16KB)
  if (addr >= BX_RAGE128_THIS pci_bar[2].addr &&
      addr < (BX_RAGE128_THIS pci_bar[2].addr + 0x4000)) {
...
#endif
  for (unsigned i = 0; i < len; i++) {
    *data_ptr = BX_RAGE128_THIS mem_read(addr);
    addr++;
#ifdef BX_LITTLE_ENDIAN
    data_ptr++;
#else // BX_BIG_ENDIAN
...
{
  if (BX_RAGE128_THIS pci_rom_size > 0) {
    Bit32u mask = (BX_RAGE128_THIS pci_rom_size - 1);
    if (((Bit32u)addr & ~mask) == BX_RAGE128_THIS pci_rom_address) {
      if (BX_RAGE128_THIS pci_conf[0x30] & 0x01) {
        if (BX_RAGE128_THIS pci_conf[0x50] == 0x00)
          return BX_RAGE128_THIS pci_rom[addr & mask];
        else
          return BX_RAGE128_THIS s.memory[addr & mask];
      } else {
        return 0xff;
      }
    }
  ...
  // BAR0 (LFB) is handled by rage128_mem_read_handler
  // BAR1 is I/O space, not memory space

  // Check CRTC_GEN_CNTL bit 24 (CRTC_EXT_DISP_EN): 0=VGA mode, 1=Extended mode
  // In VGA mode, use base VGA core memory read
  if (!(BX_RAGE128_THIS crtc_gen_cntl & (1 << 24)))
    return BX_RAGE128_THIS bx_vgacore_c::mem_read(addr);

  if (addr >= 0xA0000 && addr <= 0xAFFFF) {
    Bit32u offset = addr & 0xffff;
    offset += BX_RAGE128_THIS bank_base[0];
    offset &= (BX_RAGE128_THIS s.memsize - 1);
    return BX_RAGE128_THIS s.memory[offset];
  }

  return 0xFF;
}

bool bx_rage128_c::rage128_mem_write_handler(bx_phy_address addr, unsigned len,
                                         void *data, void *param)
{
  // BAR0: Linear Frame Buffer (LFB) - VRAM access
  if (addr >= BX_RAGE128_THIS pci_bar[0].addr &&
    ...
    Bit8u *data_ptr = (Bit8u *) data;
    for (unsigned i = 0; i < len; i++) {
      BX_RAGE128_THIS s.memory[offset + i] = data_ptr[i];
    }
...
#endif
  for (unsigned i = 0; i < len; i++) {
    BX_RAGE128_THIS mem_write(addr, *data_ptr);
    addr++;
#ifdef BX_LITTLE_ENDIAN
    data_ptr++;
#else // BX_BIG_ENDIAN
...
void bx_rage128_c::mem_write(bx_phy_address addr, Bit8u value)
{
  if ((addr >= BX_RAGE128_THIS pci_bar[0].addr) &&
    ...
    BX_RAGE128_THIS s.memory[offset] = value;
    if (BX_RAGE128_THIS svga_pitch != 0) {
      BX_RAGE128_THIS svga_needs_update_tile = 1;
      offset -= (Bit32u)(BX_RAGE128_THIS disp_ptr - BX_RAGE128_THIS s.memory);
      x = (offset % BX_RAGE128_THIS svga_pitch) / (BX_RAGE128_THIS svga_bpp / 8);
      y = offset / BX_RAGE128_THIS svga_pitch;
      SET_TILE_UPDATED(BX_RAGE128_THIS, x / X_TILESIZE, y / Y_TILESIZE, 1);
    }
    return;
  }
  ...
  if (!(BX_RAGE128_THIS crtc_gen_cntl & (1 << 24))) {
    BX_RAGE128_THIS bx_vgacore_c::mem_write(addr, value);
    return;
  }
  ...
  if (addr >= 0xA0000 && addr <= 0xAFFFF) {
    ...
    if (BX_RAGE128_THIS crtc.reg[0x1c] & 0x80) {
      BX_RAGE128_THIS s.memory[offset] = value;
      return;
    }
    offset += BX_RAGE128_THIS bank_base[0];
...
void bx_rage128_c::get_text_snapshot(Bit8u **text_snapshot,
...
Bit32u bx_rage128_c::svga_read(Bit32u address, unsigned io_len)
{
  ...
  switch (address) {
    ...
    case 0x03d4: /* VGA: CRTC Index Register (color emulation modes) */
      return BX_RAGE128_THIS crtc.index;
    ...
    case 0x03d5: /* VGA: CRTC Registers (color emulation modes) */
      ...
      if (BX_RAGE128_THIS crtc.index > VGA_CRTC_MAX && BX_RAGE128_THIS crtc.index <= RAGE128_CRTC_MAX)
        return BX_RAGE128_THIS svga_read_crtc(address,BX_RAGE128_THIS crtc.index);
      else
        break;
...
void bx_rage128_c::svga_write(Bit32u address, Bit32u value, unsigned io_len)
{
  ...
  switch (address) {
    case 0x03b4: /* VGA: CRTC Index Register (monochrome emulation modes) */
    case 0x03d4: /* VGA: CRTC Index Register (color emulation modes) */
      BX_RAGE128_THIS crtc.index = value;
      break;
    case 0x03b5: /* VGA: CRTC Registers (monochrome emulation modes) */
    case 0x03d5: /* VGA: CRTC Registers (color emulation modes) */
      // Standard VGA CRTC registers (0x00-0x18)
      if (BX_RAGE128_THIS crtc.index <= VGA_CRTC_MAX) {
        BX_RAGE128_THIS crtc.reg[BX_RAGE128_THIS crtc.index] = value;
        // ATI Rage 128 PRO standard VGA CRTC registers that trigger mode update
        if (BX_RAGE128_THIS crtc.index == 0x01 ||  // H_DISP_END
            BX_RAGE128_THIS crtc.index == 0x07 ||  // CRTC Overflow
            BX_RAGE128_THIS crtc.index == 0x09 ||  // MAX_ROW_SCAN
            BX_RAGE128_THIS crtc.index == 0x0c ||  // START_ADDR_H
            BX_RAGE128_THIS crtc.index == 0x0d ||  // START_ADDR_L
            BX_RAGE128_THIS crtc.index == 0x12 ||  // V_DISP_END
            BX_RAGE128_THIS crtc.index == 0x13 ||  // OFFSET
            BX_RAGE128_THIS crtc.index == 0x15) {  // UNDERLINE_LOC
          BX_RAGE128_THIS svga_needs_update_mode = 1;
        }
        // Translate VGA CRTC registers to accelerator CRTC registers
        BX_RAGE128_THIS svga_write_crtc(address, BX_RAGE128_THIS crtc.index, value);
      } 
      // Extended ATI Rage 128 PRO CRTC registers (0x19-0x9F)
      else if (BX_RAGE128_THIS crtc.index <= RAGE128_CRTC_MAX) {
        BX_RAGE128_THIS crtc.reg[BX_RAGE128_THIS crtc.index] = value;
        // CRTC 0x19 (MODE_CNTL) - check if extended mode should be enabled
        if (BX_RAGE128_THIS crtc.index == 0x19) {
          // If bit 7 is set, enable extended display mode
          // This is a common pattern for ATI cards to switch from VGA to extended mode
          if (value & 0x80) {
            BX_RAGE128_THIS crtc_gen_cntl |= (1 << 24); // Set CRTC_EXT_DISP_EN
          }
          BX_RAGE128_THIS svga_needs_update_mode = 1;
        }
        BX_RAGE128_THIS svga_write_crtc(address, BX_RAGE128_THIS crtc.index, value);
...
void bx_rage128_c::draw_hardware_cursor(unsigned xc, unsigned yc, bx_svga_tileinfo_t *info)
{
  ...
  bool cursor_enabled = (BX_RAGE128_THIS crtc_gen_cntl & (1 << 16)) != 0;
  if (!cursor_enabled) {
    return;
  }
  
  // Check cursor mode (bits 19:17), mode 0 = 2bpp monochrome 64x64
  Bit32u cursor_mode = (BX_RAGE128_THIS crtc_gen_cntl >> 17) & 0x7;
  if (cursor_mode != 0) {
  ...
  Bit32u cur_horz_pos = (BX_RAGE128_THIS crtc_cur_hv_pos >> 16) & 0x7FF;
  Bit32u cur_vert_pos = BX_RAGE128_THIS crtc_cur_hv_pos & 0x7FF;
  ...
  Bit32u cur_horz_off = (BX_RAGE128_THIS crtc_cur_hv_offs >> 16) & 0x3F;
  Bit32u cur_vert_off = BX_RAGE128_THIS crtc_cur_hv_offs & 0x3F;
  ...
  Bit32u cursor_offset = BX_RAGE128_THIS crtc_cur_offset & 0xFFFFFFF0;
  
  // CUR_CLR0 and CUR_CLR1: Cursor colors (24-bit RGB)
  Bit8u cur_clr0_r = (BX_RAGE128_THIS crtc_cur_color0 >> 16) & 0xFF;
  Bit8u cur_clr0_g = (BX_RAGE128_THIS crtc_cur_color0 >> 8) & 0xFF;
  Bit8u cur_clr0_b = BX_RAGE128_THIS crtc_cur_color0 & 0xFF;
  Bit8u cur_clr1_r = (BX_RAGE128_THIS crtc_cur_color1 >> 16) & 0xFF;
  Bit8u cur_clr1_g = (BX_RAGE128_THIS crtc_cur_color1 >> 8) & 0xFF;
  Bit8u cur_clr1_b = BX_RAGE128_THIS crtc_cur_color1 & 0xFF;
  ...
  if (info->snapshot_mode) {
    ...
    w = BX_RAGE128_THIS svga_xres;
    h = BX_RAGE128_THIS svga_yres;
  } else {
  ...
  if ((int)xc < hwcx + (int)cursor_width &&
    ...
    Bit8u display_color_bytes = BX_RAGE128_THIS svga_bpp >> 3;
    ...
    Bit8u* cursor_data = BX_RAGE128_THIS s.memory + cursor_offset + cursor_row * cursor_pitch;
    
    Bit8u* vid_ptr = BX_RAGE128_THIS disp_ptr + BX_RAGE128_THIS svga_pitch * cy0;
    ...
    for (unsigned cy = cy0; cy < cy1; cy++) {
      ...
      vid_ptr += BX_RAGE128_THIS svga_pitch;
    }
  }
}

Bit16u bx_rage128_c::get_crtc_vtotal()
{
  ...
  if (!(BX_RAGE128_THIS crtc_gen_cntl & (1 << 24))) {
    ...
    return (Bit16u)(BX_RAGE128_THIS crtc.reg[6] +
                     ((BX_RAGE128_THIS crtc.reg[7] & 0x01) << 8) +
                     ((BX_RAGE128_THIS crtc.reg[7] & 0x20) << 4) + 2);
  }
  ...
  return (Bit16u)(BX_RAGE128_THIS crtc_v_total_disp & 0x7FF) + 2;
}
...
void bx_rage128_c::get_crtc_params(bx_crtc_params_t* crtcp, Bit32u* vclock)
{
  ...
  Bit32u m = BX_RAGE128_THIS pll_vclk_cntl & 0xFF;
  Bit32u n = BX_RAGE128_THIS pll_vclk_cntl >> 8 & 0xFF;
  Bit32u p = BX_RAGE128_THIS pll_vclk_cntl >> 16 & 0x07;
  Bit32u mb = 1;
  Bit32u nb = 1;
  if (BX_RAGE128_THIS pll_vclk_cntl2 & 0x00000080) {
    mb = BX_RAGE128_THIS pll_vclk_cntl2 >> 4 & 0x7;
    nb = (BX_RAGE128_THIS pll_vclk_cntl2 >> 21) & 0x18 |
         BX_RAGE128_THIS pll_vclk_cntl2 >> 19 & 0x7;
  }
  if (BX_RAGE128_THIS pll_gen_cntl & 0x200 && m && mb) {
    Bit32u crystalFreq = 13500000; // Default crystal frequency for Rage 128
    if (BX_RAGE128_THIS config_xstrap & 0x00000040)
      crystalFreq = 14318000;
    if (BX_RAGE128_THIS config_xstrap & 0x00400000)
  ...
  crtcp->htotal = (BX_RAGE128_THIS crtc_h_total_disp & 0x1FF);
  ...
  Bit32u v_sync_strt = BX_RAGE128_THIS crtc_v_sync_strt_wid & 0x7FF;
  Bit32u v_sync_wid = (BX_RAGE128_THIS crtc_v_sync_strt_wid >> 16) & 0x1F;
...
void bx_rage128_c::update(void)
{
  ...
  if (BX_RAGE128_THIS svga_needs_update_mode) {
    ...
    Bit32u iWidth = ((BX_RAGE128_THIS crtc_h_total_disp >> 16) & 0xFF) + 1;
    iWidth <<= 3; // Convert from characters to pixels
    
    // CRTC_V_TOTAL_DISP: V_DISP is bits 26:16
    // Value represents display height - 1
    Bit32u iHeight = ((BX_RAGE128_THIS crtc_v_total_disp >> 16) & 0x7FF) + 1;

    if ((iWidth != BX_RAGE128_THIS svga_xres) || (iHeight != BX_RAGE128_THIS svga_yres)
        || (iBpp != BX_RAGE128_THIS svga_bpp)) {
      BX_INFO(("switched to %u x %u x %u", iWidth, iHeight, iBpp));
    }

    BX_RAGE128_THIS svga_xres = iWidth;
    BX_RAGE128_THIS svga_yres = iHeight;
    BX_RAGE128_THIS svga_bpp = iBpp;
    BX_RAGE128_THIS svga_dispbpp = iBpp;
    BX_RAGE128_THIS disp_ptr = BX_RAGE128_THIS s.memory + iTopOffset;
    BX_RAGE128_THIS svga_pitch = iPitch;
    // compatibilty settings for VGA core
    BX_RAGE128_THIS s.last_xres = iWidth;
    BX_RAGE128_THIS s.last_yres = iHeight;
    BX_RAGE128_THIS s.last_bpp = iBpp;
    BX_RAGE128_THIS s.last_fh = 0;
  }

  if (BX_RAGE128_THIS svga_dispbpp != 4) {
    width  = BX_RAGE128_THIS svga_xres;
    height = BX_RAGE128_THIS svga_yres;
    pitch = BX_RAGE128_THIS svga_pitch;
    if (BX_RAGE128_THIS svga_needs_update_mode) {
      bx_gui->dimension_update(width, height, 0, 0, BX_RAGE128_THIS svga_dispbpp);
      BX_RAGE128_THIS s.last_bpp = BX_RAGE128_THIS svga_dispbpp;
      BX_RAGE128_THIS svga_needs_update_mode = 0;
      BX_RAGE128_THIS svga_needs_update_dispentire = 1;
    }
  } else {
    BX_RAGE128_THIS determine_screen_dimensions(&height, &width);
    pitch = BX_RAGE128_THIS s.line_offset;
    if ((width != BX_RAGE128_THIS s.last_xres) || (height != BX_RAGE128_THIS s.last_yres) ||
        (BX_RAGE128_THIS s.last_bpp > 8)) {
      bx_gui->dimension_update(width, height);
      BX_RAGE128_THIS s.last_xres = width;
      BX_RAGE128_THIS s.last_yres = height;
      BX_RAGE128_THIS s.last_bpp = 8;
    }
  }

  if (BX_RAGE128_THIS svga_needs_update_dispentire) {
    BX_RAGE128_THIS redraw_area(0,0,width,height);
    BX_RAGE128_THIS svga_needs_update_dispentire = 0;
  }

  if (!BX_RAGE128_THIS svga_needs_update_tile) {
    return;
  }
  BX_RAGE128_THIS svga_needs_update_tile = 0;
  ...
  if (bx_gui->graphics_tile_info_common(&info)) {
    if (info.snapshot_mode) {
      vid_ptr = BX_RAGE128_THIS disp_ptr;
      tile_ptr = bx_gui->get_snapshot_buffer();
      if (tile_ptr != NULL) {
        for (yc = 0; yc < height; yc++) {
          if (BX_RAGE128_THIS svga_dispbpp != 4) {
            ...
            tile_ptr2 = tile_ptr;
            row_addr = BX_RAGE128_THIS s.CRTC.start_addr + (yc * pitch);
            for (xc = 0; xc < width; xc++) {
              *(tile_ptr2++) = BX_RAGE128_THIS get_vga_pixel(xc, yc, row_addr, 0xffff, 0, BX_RAGE128_THIS s.memory);
            }
          }
          tile_ptr += info.pitch;
        }
      ...
      switch (BX_RAGE128_THIS svga_dispbpp) {
        case 4:
          for (yc=0, yti = 0; yc<height; yc+=Y_TILESIZE, yti++) {
            for (xc=0, xti = 0; xc<width; xc+=X_TILESIZE, xti++) {
              if (GET_TILE_UPDATED (xti, yti)) {
                ...
                for (r=0; r<h; r++) {
                  y = yc + r;
                  if (BX_RAGE128_THIS s.y_doublescan) y >>= 1;
                  row_addr = BX_RAGE128_THIS s.CRTC.start_addr + (y * pitch);
                  tile_ptr2 = tile_ptr;
                  for (c=0; c<w; c++) {
                    x = xc + c;
                    *(tile_ptr2++) = BX_RAGE128_THIS get_vga_pixel(x, y, row_addr, 0xffff, 0, BX_RAGE128_THIS s.memory);
                  }
                  tile_ptr += info.pitch;
                }
        ...
        case 32:
          BX_ERROR(("current guest pixel format is unsupported on indexed colour host displays, svga_dispbpp=%d",
            BX_RAGE128_THIS svga_dispbpp));
          break;
        case 8:
          hp = BX_RAGE128_THIS s.attribute_ctrl.horiz_pel_panning & 0x07;
          for (yc=0, yti = 0; yc<height; yc+=Y_TILESIZE, yti++) {
            for (xc=0, xti = 0; xc<width; xc+=X_TILESIZE, xti++) {
              if (GET_TILE_UPDATED (xti, yti)) {
                if (!BX_RAGE128_THIS s.y_doublescan) {
                  vid_ptr = BX_RAGE128_THIS disp_ptr + (yc * pitch + xc + hp);
                } else {
                  if (!BX_RAGE128_THIS svga_double_width) {
                    vid_ptr = BX_RAGE128_THIS disp_ptr + ((yc >> 1) * pitch + xc + hp);
                  } else {
                    vid_ptr = BX_RAGE128_THIS disp_ptr + ((yc >> 1) * pitch + ((xc + hp) >> 1));
                  }
                }
                tile_ptr = bx_gui->graphics_tile_get(xc, yc, &w, &h);
                for (r=0; r<h; r++) {
                  ...
                  for (c=0; c<w; c++) {
                    colour = *(vid_ptr2);
                    if (!BX_RAGE128_THIS svga_double_width || (c & 1)) {
                      vid_ptr2++;
                    }
                    *(tile_ptr2++) = colour;
                  }
                  if (!BX_RAGE128_THIS s.y_doublescan || (r & 1)) {
                    vid_ptr += pitch;
                  }
                  tile_ptr += info.pitch;
                }
      ...
      switch (BX_RAGE128_THIS svga_dispbpp) {
              ...
              if (GET_TILE_UPDATED (xti, yti)) {
                ...
                for (r=0; r<Y_TILESIZE; r++) {
                  ...
                  for (c=0; c<X_TILESIZE; c++) {
                    x = xc + c;
                    colour = BX_RAGE128_THIS get_vga_pixel(x, y, row_addr, 0xffff, 0, BX_RAGE128_THIS s.memory);
                    colour = MAKE_COLOUR(
                      BX_RAGE128_THIS s.pel.data[colour].red, 6, info.red_shift, info.red_mask,
                      BX_RAGE128_THIS s.pel.data[colour].green, 6, info.green_shift, info.green_mask,
                      BX_RAGE128_THIS s.pel.data[colour].blue, 6, info.blue_shift, info.blue_mask);
                    if (info.is_little_endian) {
        ...
        case 8:
          ...
          break;
        case 15:
          hp = BX_RAGE128_THIS s.attribute_ctrl.horiz_pel_panning & 0x01;
          for (yc=0, yti = 0; yc<height; yc+=Y_TILESIZE, yti++) {
            for (xc=0, xti = 0; xc<width; xc+=X_TILESIZE, xti++) {
              if (GET_TILE_UPDATED (xti, yti)) {
                if (!BX_RAGE128_THIS s.y_doublescan) {
                  vid_ptr = BX_RAGE128_THIS disp_ptr + (yc * pitch + ((xc + hp) << 1));
                } else {
                  if (!BX_RAGE128_THIS svga_double_width) {
                    vid_ptr = BX_RAGE128_THIS disp_ptr + ((yc >> 1) * pitch + ((xc + hp) << 1));
                  } else {
        ...
        case 24:
          for (yc=0, yti = 0; yc<height; yc+=Y_TILESIZE, yti++) {
            for (xc=0, xti = 0; xc<width; xc+=X_TILESIZE, xti++) {
              if (GET_TILE_UPDATED (xti, yti)) {
                if (!BX_RAGE128_THIS s.y_doublescan) {
                  vid_ptr = BX_RAGE128_THIS disp_ptr + (yc * pitch + 3 * xc);
                } else {
                  if (!BX_RAGE128_THIS svga_double_width) {
                    vid_ptr = BX_RAGE128_THIS disp_ptr + ((yc >> 1) * pitch + 3 * xc);
                  } else {
                    vid_ptr = BX_RAGE128_THIS disp_ptr + ((yc >> 1) * pitch + 3 * (xc >> 1));
                  }
                }
                tile_ptr = bx_gui->graphics_tile_get(xc, yc, &w, &h);
                for (r=0; r<h; r++) {
        ...
        case 32:
          for (yc=0, yti = 0; yc<height; yc+=Y_TILESIZE, yti++) {
            for (xc=0, xti = 0; xc<width; xc+=X_TILESIZE, xti++) {
              if (GET_TILE_UPDATED (xti, yti)) {
                if (!BX_RAGE128_THIS s.y_doublescan) {
                  vid_ptr = BX_RAGE128_THIS disp_ptr + (yc * pitch + (xc << 2));
                } else {
                  if (!BX_RAGE128_THIS svga_double_width) {
                    vid_ptr = BX_RAGE128_THIS disp_ptr + ((yc >> 1) * pitch + (xc << 2));
                  } else {
                    vid_ptr = BX_RAGE128_THIS disp_ptr + ((yc >> 1) * pitch + (xc << 1));
                  }
                }
                tile_ptr = bx_gui->graphics_tile_get(xc, yc, &w, &h);
                for (r=0; r<h; r++) {
...
Bit8u bx_rage128_c::svga_read_crtc(Bit32u address, unsigned index)
{
  ...
  if (index <= VGA_CRTC_MAX) {
    ...
    return BX_RAGE128_THIS crtc.reg[index];
  }
  ...
  Bit8u value = BX_RAGE128_THIS crtc.reg[index];
  BX_DEBUG(("crtc: extended index 0x%02x read 0x%02x", index, value));
  return value;
}

void bx_rage128_c::svga_write_crtc(Bit32u address, unsigned index, Bit8u value)
{
  // ATI Rage 128 PRO CRTC registers
  ...
  if (index <= VGA_CRTC_MAX) {
    ...
    BX_RAGE128_THIS crtc.reg[index] = value;
    ...
    if (index == 0x00 || index == 0x01 || index == 0x02 || index == 0x03 || 
      ...
      Bit32u h_total = (BX_RAGE128_THIS crtc.reg[0x00] + 5) & 0x1FF;
      Bit32u h_disp = BX_RAGE128_THIS crtc.reg[0x01] & 0xFF;
      BX_RAGE128_THIS crtc_h_total_disp = h_total | (h_disp << 16);
      ...
      Bit32u h_sync_strt = (BX_RAGE128_THIS crtc.reg[0x04] & 0xFF) << 3;
      Bit32u h_sync_wid = (BX_RAGE128_THIS crtc.reg[0x05] & 0x1F) << 16;
      BX_RAGE128_THIS crtc_h_sync_strt_wid = h_sync_strt | h_sync_wid;
    }
    ...
    if (index == 0x06 || index == 0x07 || index == 0x12) {
      ...
      Bit32u v_total = (BX_RAGE128_THIS crtc.reg[0x06] & 0xFF) |
                       ((BX_RAGE128_THIS crtc.reg[0x07] & 0x01) << 8) |
                       ((BX_RAGE128_THIS crtc.reg[0x07] & 0x20) << 4);
      v_total = (v_total + 2) & 0x7FF; // Add 2, mask to 11 bits
      
      // V_DISP is in CRTC 0x12 (bits 7:0) + CRTC 0x07 (bits 1, 6), value - 1, bits 26:16
      Bit32u v_disp = (BX_RAGE128_THIS crtc.reg[0x12] & 0xFF) |
                      ((BX_RAGE128_THIS crtc.reg[0x07] & 0x02) << 7) |
                      ((BX_RAGE128_THIS crtc.reg[0x07] & 0x40) << 2);
      v_disp = (v_disp + 1) & 0x7FF; // Add 1, mask to 11 bits
      
      BX_RAGE128_THIS crtc_v_total_disp = v_total | (v_disp << 16);
    }
    ...
    if (index == 0x10 || index == 0x11 || index == 0x07) {
      ...
      Bit32u v_sync_strt = (BX_RAGE128_THIS crtc.reg[0x10] & 0xFF) |
                           ((BX_RAGE128_THIS crtc.reg[0x07] & 0x04) << 6) |
                           ((BX_RAGE128_THIS crtc.reg[0x07] & 0x80) << 2);
      v_sync_strt &= 0x7FF; // Mask to 11 bits
      
      // V_SYNC_WID is in CRTC 0x11 (bits 3:0), needs to be in bits 20:16
      Bit32u v_sync_wid = (BX_RAGE128_THIS crtc.reg[0x11] & 0x0F) << 16;
      
      BX_RAGE128_THIS crtc_v_sync_strt_wid = v_sync_strt | v_sync_wid;
    }
    ...
    if (index == 0x13) {
      ...
      Bit32u pitch = BX_RAGE128_THIS crtc.reg[0x13] & 0xFF;
      if (BX_RAGE128_THIS crtc.reg[0x1B] & 0x1F) {
        pitch |= ((BX_RAGE128_THIS crtc.reg[0x1B] & 0x1F) << 8);
      }
      ...
      BX_RAGE128_THIS crtc_pitch = (pitch & 0x3FF); // Mask to 10 bits
    } else if (index == 0x1B) {
    ...
    if (index == 0x0C || index == 0x0D) {
      ...
      Bit32u start_addr = (BX_RAGE128_THIS crtc.reg[0x0C] << 8) | BX_RAGE128_THIS crtc.reg[0x0D];
      ...
      BX_RAGE128_THIS crtc_offset = start_addr * 8; // Assume 8bpp for now
    }
...
Bit8u bx_rage128_c::register_read8(Bit32u address)
{
  Bit8u value;
  ...
  if (address >= 0x0F00 && address < 0x1000) {
    ...
    if (offset < 256) {
      value = BX_RAGE128_THIS pci_conf[offset];
    } else {
      value = 0x00;
    }
  }
...
void bx_rage128_c::register_write8(Bit32u address, Bit8u value)
{
...
Bit8u bx_rage128_c::vram_read8(Bit32u address)
{
  return BX_RAGE128_THIS s.memory[address];
}

Bit16u bx_rage128_c::vram_read16(Bit32u address)
{
  return
    BX_RAGE128_THIS s.memory[address + 0] << 0 |
    BX_RAGE128_THIS s.memory[address + 1] << 8;
}

Bit32u bx_rage128_c::vram_read32(Bit32u address)
{
  return
    BX_RAGE128_THIS s.memory[address + 0] << 0 |
    BX_RAGE128_THIS s.memory[address + 1] << 8 |
    BX_RAGE128_THIS s.memory[address + 2] << 16 |
    BX_RAGE128_THIS s.memory[address + 3] << 24;
}

void bx_rage128_c::vram_write8(Bit32u address, Bit8u value)
{
  BX_RAGE128_THIS s.memory[address + 0] = value;
}

void bx_rage128_c::vram_write16(Bit32u address, Bit16u value)
{
  BX_RAGE128_THIS s.memory[address + 0] = value >> 0 & 0xFF;
  BX_RAGE128_THIS s.memory[address + 1] = value >> 8 & 0xFF;
}

void bx_rage128_c::vram_write32(Bit32u address, Bit32u value)
{
  BX_RAGE128_THIS s.memory[address + 0] = value >> 0 & 0xFF;
  BX_RAGE128_THIS s.memory[address + 1] = value >> 8 & 0xFF;
  BX_RAGE128_THIS s.memory[address + 2] = value >> 16 & 0xFF;
  BX_RAGE128_THIS s.memory[address + 3] = value >> 24 & 0xFF;
}
...
Bit32u bx_rage128_c::get_crt_intr()
{
  Bit32u value = 0x00000000;
  if (BX_RAGE128_THIS crt_intr & BX_RAGE128_THIS crt_intr_en)
    value |= 0x00000001;
  return value;
}
...
{
  DEV_pci_set_irq(BX_RAGE128_THIS devfunc, BX_RAGE128_THIS pci_conf[0x3d], level);
}

Bit32u bx_rage128_c::register_read32(Bit32u address)
{
  Bit32u value = 0;
  ...
  if (address == 0x00E0) {
    value = BX_RAGE128_THIS config_cntl;
  } else if (address == 0x00E4) {
    value = BX_RAGE128_THIS config_xstrap;
  } else if (address == 0x00E8) {
    value = BX_RAGE128_THIS config_bonds;
  } else if (address == 0x00F8) {
    value = BX_RAGE128_THIS config_memsize;
  } else if (address == 0x0100) {
    value = BX_RAGE128_THIS config_aper_0_base;
  } else if (address == 0x0104) {
    value = BX_RAGE128_THIS config_aper_1_base;
  } else if (address == 0x0108) {
    value = BX_RAGE128_THIS config_reg_1_base;
  } else if (address == 0x010C) {
    value = BX_RAGE128_THIS config_reg_aper_size;
  } else if (address == 0x0110) {
    value = BX_RAGE128_THIS config_aper_size;
  }
  // GPIO registers for DDC communication
    ...
    value = BX_RAGE128_THIS gpio_vga_ddc;
    // Update DDC status bits (typically bits 2:3 for SDA/SCL status)
    Bit8u ddc_status = BX_RAGE128_THIS ddc.read();
    ...
    // Reading returns current GPIO state with DDC status
    value = BX_RAGE128_THIS gpio_dvi_ddc;
    // Update DDC status bits (typically bits 2:3 for SDA/SCL status)
    Bit8u ddc_status = BX_RAGE128_THIS ddc.read();
    value = (value & ~0x0c) | (ddc_status & 0x0c);
  }
  ...
  // Main CRTC control registers
    ...
    value = BX_RAGE128_THIS crtc_gen_cntl;
  } else if (address == 0x0054) {
    value = BX_RAGE128_THIS crtc_ext_cntl;
  } else if (address == 0x005C) {
    value = BX_RAGE128_THIS crtc_status;
  }
  // Combined CRTC timing registers (ATI Rage 128 PRO format)
  else if (address == 0x0200) {
    value = BX_RAGE128_THIS crtc_h_total_disp;
  } else if (address == 0x0204) {
    value = BX_RAGE128_THIS crtc_h_sync_strt_wid;
  } else if (address == 0x0208) {
    value = BX_RAGE128_THIS crtc_v_total_disp;
  } else if (address == 0x020C) {
    value = BX_RAGE128_THIS crtc_v_sync_strt_wid;
  }
  // CRTC display control registers
  else if (address == 0x0210) {
    value = BX_RAGE128_THIS crtc_vline_crnt_vline;
  } else if (address == 0x0214) {
    value = BX_RAGE128_THIS crtc_crnt_frame;
  } else if (address == 0x0218) {
    value = BX_RAGE128_THIS crtc_gui_trig_vline;
  } else if (address == 0x021C) {
    value = BX_RAGE128_THIS crtc_debug;
  } else if (address == 0x0224) {
    value = BX_RAGE128_THIS crtc_offset;
  } else if (address == 0x0228) {
    value = BX_RAGE128_THIS crtc_offset_cntl;
  } else if (address == 0x022C) {
    value = BX_RAGE128_THIS crtc_pitch;
  }
  // Hardware cursor registers (ATI Rage 128 PRO addresses)
  else if (address == 0x0260) {
    value = BX_RAGE128_THIS crtc_cur_offset;
  } else if (address == 0x0264) {
    value = BX_RAGE128_THIS crtc_cur_hv_pos;
  } else if (address == 0x0268) {
    value = BX_RAGE128_THIS crtc_cur_hv_offs;
  } else if (address == 0x026C) {
    value = BX_RAGE128_THIS crtc_cur_color0;
  } else if (address == 0x0270) {
    value = BX_RAGE128_THIS crtc_cur_color1;
  }
  // Clock control and PLL (indirect access via CLK_CNTL_INDEX/DATA)
  else if (address == 0x00C0) {
    value = BX_RAGE128_THIS clk_cntl_index;
  } else if (address == 0x00C4) {
    // Read from indexed PLL register
    Bit8u index = BX_RAGE128_THIS clk_cntl_index & 0x3F;
    if (index == 0x00) value = BX_RAGE128_THIS pll_ref_div;
    else if (index == 0x01) value = BX_RAGE128_THIS pll_gen_cntl;
    else if (index == 0x02) value = BX_RAGE128_THIS pll_vclk_cntl;
    else if (index == 0x03) value = BX_RAGE128_THIS pll_vclk_cntl2;
    else if (index == 0x04) value = BX_RAGE128_THIS pll_vclk_cntl3;
    else if (index == 0x05) value = BX_RAGE128_THIS pll_vclk_ecp_cntl;
    else value = 0;
  } else if (address == 0x00C8) {
    value = BX_RAGE128_THIS pll_ref_div;
  } else if (address == 0x00CC) {
    value = BX_RAGE128_THIS pll_gen_cntl;
  } else if (address == 0x00D0) {
    value = BX_RAGE128_THIS pll_vclk_cntl;
  } else if (address == 0x00D4) {
    value = BX_RAGE128_THIS pll_vclk_cntl2;
  } else if (address == 0x00D8) {
    value = BX_RAGE128_THIS pll_vclk_cntl3;
  } else if (address == 0x00DC) {
    value = BX_RAGE128_THIS pll_vclk_ecp_cntl;
  }
  // DAC registers
  else if (address == 0x00F0) {
    value = BX_RAGE128_THIS dac_palette_addr;
  } else if (address == 0x00F4) {
    value = BX_RAGE128_THIS dac_palette_data;
  }
  // PCI configuration space mirror (0x0F00-0x0FFF)
    ...
    if (offset < 256) {
      value = 
        (BX_RAGE128_THIS pci_conf[offset + 0] << 0) |
        (BX_RAGE128_THIS pci_conf[offset + 1] << 8) |
        (BX_RAGE128_THIS pci_conf[offset + 2] << 16) |
        (BX_RAGE128_THIS pci_conf[offset + 3] << 24);
    } else {
      value = 0;
    }
  }
  // GUI engine registers (0x1400-0x1FFF)
    ...
    value = BX_RAGE128_THIS gui_eng_cntl;
  } else if (address == 0x1404) {
    // DST_OFFSET (0x1404) - also used as GUI_ENG_STATUS
    value = BX_RAGE128_THIS dst_offset;
  } else if (address == 0x1408) {
    value = BX_RAGE128_THIS dst_pitch;
  } else if (address == 0x140C) {
    value = BX_RAGE128_THIS dst_width;
  } else if (address == 0x1410) {
    value = BX_RAGE128_THIS dst_height;
  } else if (address == 0x1414) {
    value = BX_RAGE128_THIS src_x;
  } else if (address == 0x1418) {
    value = BX_RAGE128_THIS src_y;
  } else if (address == 0x141C) {
    value = BX_RAGE128_THIS dst_x;
  } else if (address == 0x1420) {
    value = BX_RAGE128_THIS dst_y;
  } else if (address == 0x1428) {
    value = BX_RAGE128_THIS src_pitch_offset;
  } else if (address == 0x142C) {
    value = BX_RAGE128_THIS dst_pitch_offset;
  } else if (address == 0x1434) {
    value = BX_RAGE128_THIS src_y_x;
  } else if (address == 0x1438) {
    value = BX_RAGE128_THIS dst_y_x;
  } else if (address == 0x146C) {
    value = BX_RAGE128_THIS dp_gui_master_cntl;
  } else if (address == 0x15AC) {
    value = BX_RAGE128_THIS src_offset;
  } else if (address == 0x15B0) {
    value = BX_RAGE128_THIS src_pitch;
  } else if (address == 0x15D8) {
    value = BX_RAGE128_THIS dp_src_frgd_clr;
  } else if (address == 0x15DC) {
    value = BX_RAGE128_THIS dp_src_bkgd_clr;
  } else if (address == 0x16C0) {
    value = BX_RAGE128_THIS dp_cntl;
  } else if (address == 0x16C4) {
    value = BX_RAGE128_THIS dp_datatype;
  } else if (address == 0x16C8) {
    value = BX_RAGE128_THIS dp_mix;
  } else if (address >= 0x17C0 && address <= 0x17DC) {
    // HOST_DATA[0-7] registers
    Bit32u index = (address - 0x17C0) / 4;
    if (index < 8) {
      value = BX_RAGE128_THIS host_data[index];
    } else {
      value = 0;
    }
  } else if (address == 0x17E0) {
    value = BX_RAGE128_THIS host_data_last;
  }
  // Interrupt registers
  else if (address == 0x00A0) { // CRT_INTR (approximate location)
    value = BX_RAGE128_THIS crt_intr;
  } else if (address == 0x00A4) { // CRT_INTR_EN (approximate location)
    value = BX_RAGE128_THIS crt_intr_en;
  }
  // Unknown/unimplemented registers - use regs array
  else if (address < RAGE128_REG_APER_SIZE) {
    value = BX_RAGE128_THIS regs[address / 4];
  } else {
    value = 0;
  }
...
void bx_rage128_c::register_write32(Bit32u address, Bit32u value)
{
  // ATI Rage 128 register map
  // Configuration registers
  if (address == 0x00E0) {
    BX_RAGE128_THIS config_cntl = value;
  } else if (address == 0x00E4) {
    BX_RAGE128_THIS config_xstrap = value;
  } else if (address == 0x00E8) {
    BX_RAGE128_THIS config_bonds = value;
  } else if (address == 0x00F8) {
    BX_RAGE128_THIS config_memsize = value;
  } else if (address == 0x0100) {
    BX_RAGE128_THIS config_aper_0_base = value;
  } else if (address == 0x0104) {
    BX_RAGE128_THIS config_aper_1_base = value;
  } else if (address == 0x0108) {
    BX_RAGE128_THIS config_reg_1_base = value;
  } else if (address == 0x010C) {
    BX_RAGE128_THIS config_reg_aper_size = value;
  } else if (address == 0x0110) {
    BX_RAGE128_THIS config_aper_size = value;
  }
  // GPIO registers for DDC communication
  else if (address == 0x0060) {
    // GPIO_VGA_DDC: VGA DDC I2C control register
    ...
    BX_RAGE128_THIS gpio_vga_ddc = value;
    ...
    if (scl_en && sda_en) {
      BX_RAGE128_THIS ddc.write(scl, sda);
    }
  } else if (address == 0x0064) {
    // GPIO_DVI_DDC: DVI DDC I2C control register
    // Bits typically control SCL/SDA for DVI DDC communication
    ...
    // Bit 3: SDA data (when output enabled)
    // Bits 4+: Other GPIO functions
    BX_RAGE128_THIS gpio_dvi_ddc = value;
    // Extract SCL/SDA from GPIO register and write to DDC
  ...
  // Main CRTC control registers
    ...
    BX_RAGE128_THIS crtc_gen_cntl = value;
    BX_RAGE128_THIS svga_needs_update_mode = 1;
    // Update cursor enable state (bit 16)
    bool prev_enabled = BX_RAGE128_THIS hw_cursor.enabled;
    BX_RAGE128_THIS hw_cursor.enabled = (value & (1 << 16)) != 0;
    if (prev_enabled != BX_RAGE128_THIS hw_cursor.enabled) {
      ...
      Bit16s hwcy = (Bit16s)cur_vert_pos - (Bit16s)cur_vert_off;
      Bit32u cursor_height = 64 - cur_vert_off;
      BX_RAGE128_THIS redraw_area(hwcx, hwcy, 64, cursor_height);
    }
  } else if (address == 0x0054) {
    BX_RAGE128_THIS crtc_ext_cntl = value;
  } else if (address == 0x005C) {
    BX_RAGE128_THIS crtc_status = value;
  }
  // Combined CRTC timing registers (ATI Rage 128 PRO format)
  else if (address == 0x0200) {
    BX_RAGE128_THIS crtc_h_total_disp = value;
  } else if (address == 0x0204) {
    BX_RAGE128_THIS crtc_h_sync_strt_wid = value;
  } else if (address == 0x0208) {
    BX_RAGE128_THIS crtc_v_total_disp = value;
  } else if (address == 0x020C) {
    BX_RAGE128_THIS crtc_v_sync_strt_wid = value;
  }
  // CRTC display control registers
  else if (address == 0x0210) {
    BX_RAGE128_THIS crtc_vline_crnt_vline = value;
  } else if (address == 0x0214) {
    BX_RAGE128_THIS crtc_crnt_frame = value;
  } else if (address == 0x0218) {
    BX_RAGE128_THIS crtc_gui_trig_vline = value;
  } else if (address == 0x021C) {
    BX_RAGE128_THIS crtc_debug = value;
  } else if (address == 0x0224) {
    BX_RAGE128_THIS crtc_offset = value;
  } else if (address == 0x0228) {
    BX_RAGE128_THIS crtc_offset_cntl = value;
  } else if (address == 0x022C) {
    BX_RAGE128_THIS crtc_pitch = value;
    // CRTC_PITCH: Display line pitch in (pixels * 8), bits 9:0
    BX_RAGE128_THIS svga_pitch = (value & 0x3FF) << 3;
  }
  // Hardware cursor registers (ATI Rage 128 PRO addresses)
  // CUR_OFFSET (0x0260): Hardware cursor address offset in frame buffer
  else if (address == 0x0260) {
    BX_RAGE128_THIS crtc_cur_offset = value;
    // Bits 3:0 are hardwired to ZERO
    BX_RAGE128_THIS crtc_cur_offset &= 0xFFFFFFF0;
  }
  // CUR_HORZ_VERT_POSN (0x0264): Cursor position
  // CUR_HORZ_POSN is bits 26:16, CUR_VERT_POSN is bits 10:0
  else if (address == 0x0264) {
    BX_RAGE128_THIS crtc_cur_hv_pos = value;
    // Update cursor position (accounting for offsets)
    Bit32u cur_horz_pos = (value >> 16) & 0x7FF;
    Bit32u cur_vert_pos = value & 0x7FF;
    Bit32u cur_horz_off = (BX_RAGE128_THIS crtc_cur_hv_offs >> 16) & 0x3F;
    Bit32u cur_vert_off = BX_RAGE128_THIS crtc_cur_hv_offs & 0x3F;
    BX_RAGE128_THIS hw_cursor.x = (Bit16s)cur_horz_pos - (Bit16s)cur_horz_off;
    BX_RAGE128_THIS hw_cursor.y = (Bit16s)cur_vert_pos - (Bit16s)cur_vert_off;
  }
  // CUR_HORZ_VERT_OFF (0x0268): Cursor offsets
  // CUR_HORZ_OFF is bits 21:16, CUR_VERT_OFF is bits 5:0
  else if (address == 0x0268) {
    BX_RAGE128_THIS crtc_cur_hv_offs = value;
    // Update cursor position (accounting for offsets)
    Bit32u cur_horz_pos = (BX_RAGE128_THIS crtc_cur_hv_pos >> 16) & 0x7FF;
    Bit32u cur_vert_pos = BX_RAGE128_THIS crtc_cur_hv_pos & 0x7FF;
    ...
    BX_RAGE128_THIS hw_cursor.size = 64 - cur_vert_off;
  }
  // CUR_CLR0 (0x026C): Cursor color 0 (24-bit RGB)
  else if (address == 0x026C) {
    BX_RAGE128_THIS crtc_cur_color0 = value;
  }
  // CUR_CLR1 (0x0270): Cursor color 1 (24-bit RGB)
  else if (address == 0x0270) {
    BX_RAGE128_THIS crtc_cur_color1 = value;
  }
  // Clock control and PLL
  else if (address == 0x00C0) {
    BX_RAGE128_THIS clk_cntl_index = value;
  } else if (address == 0x00C4) {
    // Write to indexed PLL register
    Bit8u index = BX_RAGE128_THIS clk_cntl_index & 0x3F;
    if (index == 0x00) BX_RAGE128_THIS pll_ref_div = value;
    else if (index == 0x01) BX_RAGE128_THIS pll_gen_cntl = value;
    else if (index == 0x02) BX_RAGE128_THIS pll_vclk_cntl = value;
    else if (index == 0x03) BX_RAGE128_THIS pll_vclk_cntl2 = value;
    else if (index == 0x04) BX_RAGE128_THIS pll_vclk_cntl3 = value;
    else if (index == 0x05) BX_RAGE128_THIS pll_vclk_ecp_cntl = value;
    BX_RAGE128_THIS calculate_retrace_timing();
  } else if (address == 0x00C8) {
    BX_RAGE128_THIS pll_ref_div = value;
  } else if (address == 0x00CC) {
    BX_RAGE128_THIS pll_gen_cntl = value;
  } else if (address == 0x00D0) {
    BX_RAGE128_THIS pll_vclk_cntl = value;
    BX_RAGE128_THIS calculate_retrace_timing();
  } else if (address == 0x00D4) {
    BX_RAGE128_THIS pll_vclk_cntl2 = value;
  } else if (address == 0x00D8) {
    BX_RAGE128_THIS pll_vclk_cntl3 = value;
  } else if (address == 0x00DC) {
    BX_RAGE128_THIS pll_vclk_ecp_cntl = value;
  }
  // DAC registers
  else if (address == 0x00F0) {
    BX_RAGE128_THIS dac_palette_addr = value;
  } else if (address == 0x00F4) {
    BX_RAGE128_THIS dac_palette_data = value;
  }
  ...
  // GUI engine registers (0x1400-0x1FFF)
    ...
    BX_RAGE128_THIS gui_eng_cntl = value;
  } else if (address == 0x1404) {
    // DST_OFFSET (0x1404) - also used as GUI_ENG_STATUS
    BX_RAGE128_THIS dst_offset = value & 0x03FFFFFF; // Bits 25:0, bits 3:0 hardwired to zero
    BX_RAGE128_THIS dst_offset &= 0xFFFFFFF0;
  } else if (address == 0x1408) {
    // DST_PITCH (0x1408)
    BX_RAGE128_THIS dst_pitch = value;
  } else if (address == 0x140C) {
    // DST_WIDTH (0x140C) - also used as GUI_ENG_ACK
    BX_RAGE128_THIS dst_width = value;
  } else if (address == 0x1410) {
    BX_RAGE128_THIS dst_height = value;
  } else if (address == 0x1414) {
    BX_RAGE128_THIS src_x = value & 0x3FFF; // Bits 13:0
  } else if (address == 0x1418) {
    BX_RAGE128_THIS src_y = value & 0x3FFF; // Bits 13:0
  } else if (address == 0x141C) {
    BX_RAGE128_THIS dst_x = value & 0x3FFF; // Bits 13:0
  } else if (address == 0x1420) {
    BX_RAGE128_THIS dst_y = value & 0x3FFF; // Bits 13:0
  } else if (address == 0x1428) {
    BX_RAGE128_THIS src_pitch_offset = value;
  } else if (address == 0x142C) {
    BX_RAGE128_THIS dst_pitch_offset = value;
  } else if (address == 0x1434) {
    BX_RAGE128_THIS src_y_x = value;
  } else if (address == 0x1438) {
    BX_RAGE128_THIS dst_y_x = value;
  } else if (address == 0x146C) {
    BX_RAGE128_THIS dp_gui_master_cntl = value;
    ...
    BX_RAGE128_THIS execute_bitblt();
  } else if (address == 0x15AC) {
    // SRC_OFFSET (0x15AC)
    BX_RAGE128_THIS src_offset = value & 0x03FFFFFF; // Bits 25:0, bits 3:0 hardwired to zero
    BX_RAGE128_THIS src_offset &= 0xFFFFFFF0;
  } else if (address == 0x15B0) {
    BX_RAGE128_THIS src_pitch = value;
  } else if (address == 0x15D8) {
    BX_RAGE128_THIS dp_src_frgd_clr = value;
  } else if (address == 0x15DC) {
    BX_RAGE128_THIS dp_src_bkgd_clr = value;
  } else if (address == 0x16C0) {
    BX_RAGE128_THIS dp_cntl = value;
  } else if (address == 0x16C4) {
    BX_RAGE128_THIS dp_datatype = value;
  } else if (address == 0x16C8) {
    BX_RAGE128_THIS dp_mix = value;
  } else if (address >= 0x17C0 && address <= 0x17DC) {
    // HOST_DATA[0-7] registers
    Bit32u index = (address - 0x17C0) / 4;
    if (index < 8) {
      BX_RAGE128_THIS host_data[index] = value;
    }
  } else if (address == 0x17E0) {
    BX_RAGE128_THIS host_data_last = value;
    // Writing HOST_DATA_LAST triggers host-to-screen BitBlt if GUI engine is enabled
    if (BX_RAGE128_THIS dp_gui_master_cntl & 0x80000000) {
      BX_RAGE128_THIS execute_bitblt();
    }
  }
  // Interrupt registers
  else if (address == 0x00A0) { // CRT_INTR
    BX_RAGE128_THIS crt_intr &= ~value;
    set_irq_level((BX_RAGE128_THIS crt_intr & BX_RAGE128_THIS crt_intr_en) != 0);
  } else if (address == 0x00A4) { // CRT_INTR_EN
    BX_RAGE128_THIS crt_intr_en = value;
    set_irq_level((BX_RAGE128_THIS crt_intr & BX_RAGE128_THIS crt_intr_en) != 0);
  }
  // Unknown/unimplemented registers - store in regs array
  else if (address < RAGE128_REG_APER_SIZE) {
    BX_RAGE128_THIS regs[address / 4] = value;
  }
}
...
void bx_rage128_c::svga_init_pcihandlers(void)
{
  BX_RAGE128_THIS devfunc = 0x00;
  DEV_register_pci_handlers2(BX_RAGE128_THIS_PTR,
      &BX_RAGE128_THIS devfunc, BX_PLUGIN_RAGE128, "ATI Rage 128 PRO AGP", true);
  
  // ATI Rage 128 PRO: Vendor 0x1002, Device 0x5046, Subsystem 0x1002:0x0018
  BX_RAGE128_THIS init_pci_conf(0x1002, 0x5046, 0x00, 0x030000, 0x00, BX_PCI_INTA);
  ...
  BX_RAGE128_THIS init_bar_mem(0, (64 * 1024 * 1024), rage128_mem_read_handler,
                               rage128_mem_write_handler);
  // Set prefetchable bit (bit 3) after init_bar_mem
  BX_RAGE128_THIS pci_conf[0x10] |= 0x08;
  ...
  BX_RAGE128_THIS init_bar_mem(2, 0x4000, rage128_mem_read_handler,
                                 rage128_mem_write_handler);
  
  BX_RAGE128_THIS pci_rom_address = 0;
  BX_RAGE128_THIS pci_rom_read_handler = rage128_mem_read_handler;
  BX_RAGE128_THIS load_pci_rom(SIM->get_param_string(BXPN_VGA_ROM_PATH)->getptr());

  // Subsystem ID: 0x1002:0x0018 (Rage Fury Pro/Xpert 2000 Pro)
  BX_RAGE128_THIS pci_conf[0x2c] = 0x18;
  BX_RAGE128_THIS pci_conf[0x2d] = 0x00;
  BX_RAGE128_THIS pci_conf[0x2e] = 0x02;
  BX_RAGE128_THIS pci_conf[0x2f] = 0x10;

  // Capabilities pointer
  BX_RAGE128_THIS pci_conf[0x34] = 0x50;

  // Status register
  BX_RAGE128_THIS pci_conf[0x06] = 0xB0; // Cap+ 66MHz+ FastB2B+ ParErr- DEVSEL=medium
  BX_RAGE128_THIS pci_conf[0x07] = 0x02; // FastB2B+

  // AGP Capability (offset 0x50)
  BX_RAGE128_THIS pci_conf[0x50] = 0x02; // AGP capability ID
  BX_RAGE128_THIS pci_conf[0x51] = 0x5C; // Next capability pointer
  BX_RAGE128_THIS pci_conf[0x52] = 0x20; // AGP version 2.0
  BX_RAGE128_THIS pci_conf[0x53] = 0x00;
  BX_RAGE128_THIS pci_conf[0x54] = 0x07; // Status: RQ=32 Iso- ArqSz=0 Cal=0 SBA+ ITACoh- GART64- HTrans- 64bit- FW- AGP3- Rate=x1,x2,x4
  BX_RAGE128_THIS pci_conf[0x55] = 0x20;
  BX_RAGE128_THIS pci_conf[0x56] = 0x00; // Command: RQ=1 ArqSz=0 Cal=0 SBA+ AGP- GART64- 64bit- FW- Rate=<none>
  BX_RAGE128_THIS pci_conf[0x57] = 0x01;

  // Power Management Capability (offset 0x5C)
  BX_RAGE128_THIS pci_conf[0x5C] = 0x01; // PM capability ID
  BX_RAGE128_THIS pci_conf[0x5D] = 0x00; // Next capability pointer
  BX_RAGE128_THIS pci_conf[0x5E] = 0x02; // PM version 2
  BX_RAGE128_THIS pci_conf[0x5F] = 0x00;
  BX_RAGE128_THIS pci_conf[0x60] = 0x13; // PM capabilities: PMEClk- DSI- D1+ D2- AuxCurrent=0mA PME(D0-,D1-,D2-,D3hot-,D3cold-)
  BX_RAGE128_THIS pci_conf[0x61] = 0xC8;
  BX_RAGE128_THIS pci_conf[0x62] = 0x08; // PM status: D0 NoSoftRst- PME-Enable- DSel=0 DScale=0 PME-
  BX_RAGE128_THIS pci_conf[0x63] = 0x00;
}
