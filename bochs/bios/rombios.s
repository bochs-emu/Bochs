...
!BCC_EOS
! 547 static void get_hd_geometr
! 547 y();
!BCC_EOS
...
.byte	$40
.blkb	1
! 609       { 0x1372, 0x1352, 0x
.word	$1372
.word	$1352
! 609 1312, 0x1300, 0x40 },
.word	$1312
.word	$1300
.byte	$40
.blkb	1
...
mov	sp,bp
!BCC_EOS
! 1362   bios_printf(2, "apmbios " "pcibios " "pnpbios " "eltorito " "rombios32 " "\n\n");
...
push	bp
mov	bp,sp
! 1491 ype == 0 || type > 0x3) bios_printf((2 | 4 | 1), "Bad drive type\n");
...
.16B:
! 1575     *(
! 1575 (Bit8u *)(&((ebda_data_t *) 0)->ata.devices[device].type)) = (0x00);
...
jne 	.18C
.18D:
! 1621       result = !(status & 0x80) && (status & 0x40);
! Debug: and int = const $80 to unsigned short status = [S+$C-8] (used reg = )
mov	al,-6[bp]
and	al,#$80
test	al,al
...
.1C3:
! 1718      
! 1718    sectors_low = read_dword_SS(buffer+(60*2));
...
mov	-$220[bp],ax
!BCC_EOS
! 1767             cylinders = (Bit16u)
! 1767 ((Bit32u)(cylinders)*16/15);
! Debug: cast unsigned long = const 0 to unsigned short cylinders = [S+$228-$220] (used reg = )
mov	ax,-$21E[bp]
xor	bx,bx
...
mov	[$254],ax
!BCC_EOS
! 1930   *((Bit32u *)(&((ebda_data_t *) 0)->ata.trsf
! 1930 bytes)) = (0L);
...
mov	-$2A[bp],ax
!BCC_EOS
! 2316   if (_read_byte(&((ebda_data_t *) 0)->ata.devices[device].type, ebda_seg) != 0x03) {
...
export	_cdemu_init
_cdemu_init:
! 2384   Bit16u ebda_seg=get_ebda_seg();
push	bp
mov	bp,sp
dec	sp
dec	sp
...
ret
!BCC_EOS
! 2446   if(buffer[0x20]!=0x88)return 1
.31A:
...
!BCC_EOS
! 2491     case 0x03:
! 2492       *((Bit16u *)(&((ebda_data_t *) 0)->cdemu.vdevice.spt)) =
.32D:
! 2492  (36);
...
br 	.3D8
! 3037             case 0:
! 3038               comm_byte = inhibit_mouse_int_an
.3D9:
! 3038 d_events();
! Debug: func () unsigned char = inhibit_mouse_int_and_events+0 (used reg = )
call	_inhibit_mouse_int_and_events
...
!BCC_EOS
! 3095    
! 3095        mouse_driver_offset = regs.u.r16.bx;
...
.419:
! 3236                                        0x00100000L,
! 3237                                        extended_memory_size - 0x00010000L, 0, 0, 1);
! Debug: list int = const 1 (used reg = )
mov	ax,*1
push	ax
! Debug: list int = const 0 (used reg = )
...
! 3260                     case 7:
! 3261                         set_e82
! 3261 0_range(ES, regs.u.r16.di, 0x00000000L,
.421:
...
.4F7:
! 3687    
! 3687      bios_printf(4, "KBD: int09h_handler(): scancode & asciicode are zero?\n");
...
mov	$C[bp],ax
!BCC_EOS
! 3753       X = *((Bit8u *)(&((ebda_data_t *) 0)->mou
! 3753 se_data[1]));
...
mov	$A[bp],ax
!BCC_EOS
! 3754       Y = *((Bit8u *)(&((ebda_data_t *) 0)->mouse_data[2]));
...
add	sp,*8
!BCC_EOS
! 3814     _write_dword(lba_high, SI+(Bit16u)&((dpt_t
! 3814  *) 0)->sector_count2, DS);
! Debug: list unsigned short DS = [S+$1C+2] (used reg = )
push	4[bp]
...
mov	-$1B[bp],al
!BCC_EOS
! 3856     *((Bit8u *)(&((ebda_data_t *) 0)->ata.dpte.checksum)) = (checksum);
...
mov	$2B[bx],al
!BCC_EOS
! 3891         *((Bit8u *)(SI+(Bit16u
! 3891 )&((dpt_t *) 0)->dpi.t13.iface_type[4])) = ('I');
! Debug: add unsigned short = const $2C to unsigned short SI = [S+$1E+4] (used reg = )
mov	ax,6[bp]
! Debug: cast * unsigned char = const 0 to unsigned int = ax+$2C (used reg = )
mov	bx,ax
...
jb  	.554
.555:
! 3938     bios_printf(4, "int13_harddisk: f
! 3938 unction %02x, ELDL out of range %02x\n", *(((Bit8u *)&AX)+1), ( ELDX & 0x00ff ));
! Debug: and int = const $FF to unsigned short ELDX = [S+$26+$E] (used reg = )
mov	al,$10[bp]
! Debug: list unsigned char = al+0 (used reg = )
xor	ah,ah
...
je  	.572
.573:
! 3983         lba_low = ((((Bit32u)cylinder * (Bit32u)nlh) + (Bit32u)head) * (Bit32u)nlspt) + (Bit32
! 3983 u)sector - 1;
...
! 4295     default:
.609:
! 4296       bios_printf(4, "int13_cdrom: unsupported AH=%02x\n", *(((Bit8u *)&AX)+1));
.60A:
! Debug: list unsigned char AX = [S+$20+$17] (used reg = )
mov	al,$19[bp]
xor	ah,ah
...
mov	-4[bp],ax
!BCC_EOS
! 4406       ilba = *((Bit32u *)(&((ebda_data_t *) 0)->cdem
! 4406 u.ilba));
...
mov	$12[bp],al
!BCC_EOS
! 4455       switch(*((Bit8u *)(&((ebda_data_t *) 0)->cdemu
! 4455 .media))) {
mov	al,[$25B]
jmp .641
! 4456         case 0x01: *((Bit8u *)&BX) = (0x02); break;
.642:
...
mov	-$11[bp],al
!BCC_EOS
! 4953       _memcpyb(0x0042,0x0040,return_status,get_SS(),
! 4953 7);
! Debug: list int = const 7 (used reg = )
mov	ax,*7
push	ax
...
add	sp,*$A
!BCC_EOS
! 4954       if ( (return_status[0] & 0xc0) != 0 ) {
! Debug: and int = const $C0 to unsigned char return_status = [S+$24-$19] (used reg = )
mov	al,-$17[bp]
and	al,#$C0
! Debug: ne int = const 0 to unsigned char = al+0 (used reg = )
...
!BCC_EOS
! 5034         default:
! 5035           bios_printf((2 | 4 | 1), "floppy: int13: bad floppy
.705:
! 5035  type\n");
...
.771:
! 5298    
! 5298  *(((Bit8u *)&timeout)+1) = *((Bit8u *)(0x0478 + regs.u.r16.dx));
...
add	sp,*4
!BCC_EOS
! 5606       iret_addr.flags.u.r8.flagsl &= 0xfe;
! Debug: andab int = const $FE to unsigned char iret_addr = [S+4+$18] (used reg = )
mov	al,$1A[bp]
and	al,#$FE
mov	$1A[bp],al
...
! 8515 endasm
!BCC_ENDASM
! 8516 static Bit8u vgafont8[128
! Register BX used in function int70_function
! 8516 *8]=
! 8517 {
.data
_vgafont8:
! 8518  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	0
.byte	0
! 8519  0x7e, 0x81, 0xa5, 0x81, 0xbd, 0x99, 0x81, 0x7e,
.byte	$7E
.byte	$81
.byte	$A5
.byte	$81
...
.byte	$81
.byte	$7E
! 8520  0x7e, 0xff, 0xdb, 0xff, 0xc3, 0xe7, 0xff, 0x7e,
.byte	$7E
.byte	$FF
.byte	$DB
.byte	$FF
...
.byte	$FF
.byte	$7E
! 8521  0x6c, 0xfe, 0xfe, 0xfe, 0x7c, 0x38, 0x10, 0x00,
.byte	$6C
.byte	$FE
.byte	$FE
.byte	$FE
...
.byte	$10
.byte	0
! 8522  0x10, 0x38, 0x7c, 0xfe, 0x7c, 0x38, 0x10, 0x00,
.byte	$10
.byte	$38
.byte	$7C
.byte	$FE
...
.byte	$10
.byte	0
! 8523  0x38, 0x7c, 0x38, 0xfe, 0xfe, 0x7c, 0x38, 0x7c,
.byte	$38
.byte	$7C
.byte	$38
.byte	$FE
...
.byte	$38
.byte	$7C
! 8524  0x10, 0x10, 0x38, 0x7c, 0xfe, 0x7c, 0x38, 0x7c,
.byte	$10
.byte	$10
.byte	$38
.byte	$7C
...
.byte	$38
.byte	$7C
! 8525  0x00, 0x00, 0x18, 0x3c, 0x3c, 0x18, 0x00, 0x00,
.byte	0
.byte	0
.byte	$18
.byte	$3C
...
.byte	0
.byte	0
! 8526  0xff, 0xff, 0xe7, 0xc3, 0xc3, 0xe7, 0xff, 0xff,
.byte	$FF
.byte	$FF
.byte	$E7
.byte	$C3
...
.byte	$FF
.byte	$FF
! 8527  0x00, 0x3c, 0x66, 0x42, 0x42, 0x66, 0x3c, 0x00,
.byte	0
.byte	$3C
.byte	$66
.byte	$42
...
.byte	$3C
.byte	0
! 8528  0xff, 0xc3, 0x99, 0xbd, 0xbd, 0x99, 0xc3, 0xff,
.byte	$FF
.byte	$C3
.byte	$99
.byte	$BD
...
.byte	$C3
.byte	$FF
! 8529  0x0f, 0x07, 0x0f, 0x7d, 0xcc, 0xcc, 0xcc, 0x78,
.byte	$F
.byte	7
.byte	$F
.byte	$7D
...
.byte	$CC
.byte	$78
! 8530  0x3c, 0x66, 0x66, 0x66, 0x3c, 0x18, 0x7e, 0x18,
.byte	$3C
.byte	$66
.byte	$66
.byte	$66
...
.byte	$7E
.byte	$18
! 8531  0x3f, 0x33, 0x3f, 0x30, 0x30, 0x70, 0xf0, 0xe0,
.byte	$3F
.byte	$33
.byte	$3F
.byte	$30
...
.byte	$F0
.byte	$E0
! 8532  0x7f, 0x63, 0x7f, 0x63, 0x63, 0x67, 0xe6, 0xc0,
.byte	$7F
.byte	$63
.byte	$7F
.byte	$63
...
.byte	$E6
.byte	$C0
! 8533  0x99, 0x5a, 0x3c, 0xe7, 0xe7, 0x3c, 0x5a, 0x99,
.byte	$99
.byte	$5A
.byte	$3C
.byte	$E7
...
.byte	$5A
.byte	$99
! 8534  0x80, 0xe0, 0xf8, 0xfe, 0xf8, 0xe0, 0x80, 0x00,
.byte	$80
.byte	$E0
.byte	$F8
.byte	$FE
...
.byte	$80
.byte	0
! 8535  0x02, 0x0e, 0x3e, 0xfe, 0x3e, 0x0e, 0x02, 0x00,
.byte	2
.byte	$E
.byte	$3E
.byte	$FE
...
.byte	2
.byte	0
! 8536  0x18, 0x3c, 0x7e, 0x18, 0x18, 0x7e, 0x3c, 0x18,
.byte	$18
.byte	$3C
.byte	$7E
.byte	$18
...
.byte	$3C
.byte	$18
! 8537  0x66, 0x66, 0x66, 0x66, 0x66, 0x00, 0x66, 0x00,
.byte	$66
.byte	$66
.byte	$66
.byte	$66
...
.byte	$66
.byte	0
! 8538  0x7f, 0xdb, 0xdb, 0x7b, 0x1b, 0x1b, 0x1b, 0x00,
.byte	$7F
.byte	$DB
.byte	$DB
.byte	$7B
...
.byte	$1B
.byte	0
! 8539  0x3e, 0x63, 0x38, 0x6c, 0x6c, 0x38, 0xcc, 0x78,
.byte	$3E
.byte	$63
.byte	$38
.byte	$6C
...
.byte	$CC
.byte	$78
! 8540  0x00, 0x00, 0x00, 0x00, 0x7e, 0x7e, 0x7e, 0x00,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	$7E
.byte	0
! 8541  0x18, 0x3c, 0x7e, 0x18, 0x7e, 0x3c, 0x18, 0xff,
.byte	$18
.byte	$3C
.byte	$7E
.byte	$18
...
.byte	$18
.byte	$FF
! 8542  0x18, 0x3c, 0x7e, 0x18, 0x18, 0x18, 0x18, 0x00,
.byte	$18
.byte	$3C
.byte	$7E
.byte	$18
...
.byte	$18
.byte	0
! 8543  0x18, 0x18, 0x18, 0x18, 0x7e, 0x3c, 0x18, 0x00,
.byte	$18
.byte	$18
.byte	$18
.byte	$18
...
.byte	$18
.byte	0
! 8544  0x00, 0x18, 0x0c, 0xfe, 0x0c, 0x18, 0x00, 0x00,
.byte	0
.byte	$18
.byte	$C
.byte	$FE
...
.byte	0
.byte	0
! 8545  0x00, 0x30, 0x60, 0xfe, 0x60, 0x30, 0x00, 0x00,
.byte	0
.byte	$30
.byte	$60
.byte	$FE
...
.byte	0
.byte	0
! 8546  0x00, 0x00, 0xc0, 0xc0, 0xc0, 0xfe, 0x00, 0x00,
.byte	0
.byte	0
.byte	$C0
.byte	$C0
...
.byte	0
.byte	0
! 8547  0x00, 0x24, 0x66, 0xff, 0x66, 0x24, 0x00, 0x00,
.byte	0
.byte	$24
.byte	$66
.byte	$FF
...
.byte	0
.byte	0
! 8548  0x00, 0x18, 0x3c, 0x7e, 0xff, 0xff, 0x00, 0x00,
.byte	0
.byte	$18
.byte	$3C
.byte	$7E
...
.byte	0
.byte	0
! 8549  0x00, 0xff, 0xff, 0x7e, 0x3c, 0x18, 0x00, 0x00,
.byte	0
.byte	$FF
.byte	$FF
.byte	$7E
...
.byte	0
.byte	0
! 8551  0x30, 0x78, 0x78, 0x30, 0x30, 0x00, 0x30, 0x00,
.byte	$30
.byte	$78
.byte	$78
.byte	$30
...
.byte	$30
.byte	0
! 8552  0x6c, 0x6c, 0x6c, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	$6C
.byte	$6C
.byte	$6C
.byte	0
...
.byte	0
.byte	0
! 8553  0x6c, 0x6c, 0xfe, 0x6c, 0xfe, 0x6c, 0x6c, 0x00,
.byte	$6C
.byte	$6C
.byte	$FE
.byte	$6C
...
.byte	$6C
.byte	0
! 8554  0x30, 0x7c, 0xc0, 0x78, 0x0c, 0xf8, 0x30, 0x00,
.byte	$30
.byte	$7C
.byte	$C0
.byte	$78
...
.byte	$30
.byte	0
! 8555  0x00, 0xc6, 0xcc, 0x18, 0x30, 0x66, 0xc6, 0x00,
.byte	0
.byte	$C6
.byte	$CC
.byte	$18
...
.byte	$C6
.byte	0
! 8556  0x38, 0x6c, 0x38, 0x76, 0xdc, 0xcc, 0x76, 0x00,
.byte	$38
.byte	$6C
.byte	$38
.byte	$76
...
.byte	$76
.byte	0
! 8557  0x60, 0x60, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	$60
.byte	$60
.byte	$C0
.byte	0
...
.byte	0
.byte	0
! 8558  0x18, 0x30, 0x60, 0x60, 0x60, 0x30, 0x18, 0x00,
.byte	$18
.byte	$30
.byte	$60
.byte	$60
...
.byte	$18
.byte	0
! 8559  0x60, 0x30, 0x18, 0x18, 0x18, 0
.byte	$60
.byte	$30
.byte	$18
.byte	$18
...
.byte	$60
.byte	0
! 8560  0x00, 0x66, 0x3c, 0xff, 0x3c, 0x66, 0x00, 0x00,
.byte	0
.byte	$66
.byte	$3C
.byte	$FF
...
.byte	0
.byte	0
! 8561  0x00, 0x30, 0x30, 0xfc, 0x30, 0x30, 0x00, 0x00,
.byte	0
.byte	$30
.byte	$30
.byte	$FC
...
.byte	0
.byte	0
! 8562  0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x30, 0x60,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	$30
.byte	$60
! 8563  0x00, 0x00, 0x00, 0xfc, 0x00, 0x00, 0x00, 0x00,
.byte	0
.byte	0
.byte	0
.byte	$FC
...
.byte	0
.byte	0
! 8564  0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x30, 0x00,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	$30
.byte	0
! 8565  0x06, 0x0c, 0x18, 0x30, 0x60, 0xc0, 0x80, 0x00,
.byte	6
.byte	$C
.byte	$18
.byte	$30
...
.byte	$80
.byte	0
! 8566  0x7c, 0xc6, 0xce, 0xde, 0xf6, 0xe6, 0x7c, 0x00,
.byte	$7C
.byte	$C6
.byte	$CE
.byte	$DE
...
.byte	$7C
.byte	0
! 8567  0x30, 0x70, 0x30, 0x30, 0x30, 0x30, 0xfc, 0x00,
.byte	$30
.byte	$70
.byte	$30
.byte	$30
...
.byte	$FC
.byte	0
! 8568  0x78, 0xcc, 0x0c, 0x38, 0x60, 0xcc, 0xfc, 0x00,
.byte	$78
.byte	$CC
.byte	$C
.byte	$38
...
.byte	$FC
.byte	0
! 8569  0x78, 0xcc, 0x0c, 0x38, 0x0c, 0xcc, 0x78, 0x00,
.byte	$78
.byte	$CC
.byte	$C
.byte	$38
...
.byte	$78
.byte	0
! 8570  0x1c, 0x3c, 0x6c, 0xcc, 0xfe, 0x0c, 0x1e, 0x00,
.byte	$1C
.byte	$3C
.byte	$6C
.byte	$CC
...
.byte	$1E
.byte	0
! 8571  0xfc, 0xc0, 0xf8, 0x0c, 0x0c, 0xcc, 0x78, 0x00,
.byte	$FC
.byte	$C0
.byte	$F8
.byte	$C
...
.byte	$78
.byte	0
! 8572  0x38, 0x60, 0xc0, 0xf8, 0xcc, 0xcc, 0x78, 0x00,
.byte	$38
.byte	$60
.byte	$C0
.byte	$F8
...
.byte	$78
.byte	0
! 8573  0xfc, 0xcc, 0x0c, 0x18, 0x30, 0x30, 0x30, 0x00,
.byte	$FC
.byte	$CC
.byte	$C
.byte	$18
...
.byte	$30
.byte	0
! 8574  0x78, 0xcc, 0xcc, 0x78, 0xcc, 0xcc, 0x78, 0x00,
.byte	$78
.byte	$CC
.byte	$CC
.byte	$78
...
.byte	$78
.byte	0
! 8575  0x78, 0xcc, 0xcc, 0x7c, 0x0c, 0x18, 0x70, 0x00,
.byte	$78
.byte	$CC
.byte	$CC
.byte	$7C
...
.byte	$70
.byte	0
! 8576  0x00, 0x30, 0x30, 0x00, 0x00, 0x30, 0x30, 0x00,
.byte	0
.byte	$30
.byte	$30
.byte	0
...
.byte	$30
.byte	0
! 8577  0x00, 0x30, 0x30, 0x00, 0x00, 0x30, 0x30, 0x60,
.byte	0
.byte	$30
.byte	$30
.byte	0
...
.byte	$30
.byte	$60
! 8578  0x18, 0x30, 0x60, 0xc0, 0x60, 0x30, 0x18, 0x00,
.byte	$18
.byte	$30
.byte	$60
.byte	$C0
...
.byte	$18
.byte	0
! 8579  0x00, 0x00, 0xfc, 0x00, 0x00, 0xfc, 0x00, 0x00,
.byte	0
.byte	0
.byte	$FC
.byte	0
...
.byte	0
.byte	0
! 8580  0x60, 0x30, 0x18, 0x0c, 0x18, 0x30, 0x60, 0x00,
.byte	$60
.byte	$30
.byte	$18
.byte	$C
...
.byte	$60
.byte	0
! 8581  0x78, 0xcc, 0x0c, 0x18, 0x30, 0x00, 0x30, 0x00,
.byte	$78
.byte	$CC
.byte	$C
.byte	$18
...
.byte	$30
.byte	0
! 8582  0x7c, 0xc6, 0xde, 0xde, 0xde, 0xc0, 0x78, 0x00,
.byte	$7C
.byte	$C6
.byte	$DE
.byte	$DE
...
.byte	$78
.byte	0
! 8583  0x30, 0x78, 0xcc, 0xcc, 0xfc, 0xcc, 0xcc, 0x00,
.byte	$30
.byte	$78
.byte	$CC
.byte	$CC
...
.byte	$CC
.byte	0
! 8584  0xfc, 0x66, 0x66, 0x7c, 0x66, 0x66, 0xfc, 0x00,
.byte	$FC
.byte	$66
.byte	$66
.byte	$7C
...
.byte	$FC
.byte	0
! 8585  0x3c, 0x66, 0xc0, 0xc0, 0xc0, 0x66, 0x3c, 0x00,
.byte	$3C
.byte	$66
.byte	$C0
.byte	$C0
...
.byte	$3C
.byte	0
! 8586  0xf8, 0x6c, 0x66, 0x66, 0x66, 0x6c, 0xf8, 0x00,
.byte	$F8
.byte	$6C
.byte	$66
.byte	$66
...
.byte	$F8
.byte	0
! 8587  0xfe, 0x62, 0x68, 0x78, 0x68, 0x62, 0xfe, 0x00,
.byte	$FE
.byte	$62
.byte	$68
.byte	$78
...
.byte	$FE
.byte	0
! 8588  0xfe, 0x62, 0x68, 0x78, 0x68, 0x60, 0xf0, 0x00,
.byte	$FE
.byte	$62
.byte	$68
.byte	$78
...
.byte	$F0
.byte	0
! 8589  0x3c, 0x66, 0xc0, 0xc0, 0xce, 0x66, 0x3e, 0x00,
.byte	$3C
.byte	$66
.byte	$C0
.byte	$C0
...
.byte	$3E
.byte	0
! 8590  0xcc, 0xcc, 0xcc, 0xfc, 0xcc, 0xcc, 0xcc, 0x00,
.byte	$CC
.byte	$CC
.byte	$CC
.byte	$FC
...
.byte	$CC
.byte	0
! 8591  0x78, 0x30, 0x30, 0x30, 0x30, 0x30, 0x78, 0x00,
.byte	$78
.byte	$30
.byte	$30
.byte	$30
...
.byte	$78
.byte	0
! 8592  0x1e, 0x0c, 0x0c, 0x0c, 0xcc, 0xcc, 0x78, 0x00,
.byte	$1E
.byte	$C
.byte	$C
.byte	$C
...
.byte	$78
.byte	0
! 8593  0xe6, 0x66, 0x6c, 0x78, 0x6c, 0x66, 0xe6, 0x00,
.byte	$E6
.byte	$66
.byte	$6C
.byte	$78
...
.byte	$E6
.byte	0
! 8594  0xf0, 0x60, 0x60, 0x60, 0x62, 0x66, 0xfe, 0x00,
.byte	$F0
.byte	$60
.byte	$60
.byte	$60
...
.byte	$FE
.byte	0
! 8595  0xc6, 0xee, 0xfe, 0xfe, 0xd6, 0xc6, 0xc6, 0x00,
.byte	$C6
.byte	$EE
.byte	$FE
.byte	$FE
...
.byte	$C6
.byte	0
! 8596  0xc6, 0xe6, 0xf6, 0xde, 0xce, 0xc6, 0xc6, 0x00,
.byte	$C6
.byte	$E6
.byte	$F6
.byte	$DE
...
.byte	$C6
.byte	0
! 8597  0x38, 0x6c, 0xc6, 0xc6, 0xc6, 0x6c, 0x38, 0x00,
.byte	$38
.byte	$6C
.byte	$C6
.byte	$C6
...
.byte	$38
.byte	0
! 8598  0xfc, 0x66, 0x66, 0x7c, 0x60, 0x60, 0xf0, 0x00,
.byte	$FC
.byte	$66
.byte	$66
.byte	$7C
...
.byte	$F0
.byte	0
! 8599  0x78, 0xcc, 0xcc, 0xcc, 0xdc, 0x78, 0x1c, 0x00,
.byte	$78
.byte	$CC
.byte	$CC
.byte	$CC
...
.byte	$1C
.byte	0
! 8600  0xfc, 0x66, 0x66, 0x7c, 0x6c, 0x66, 0xe6, 0x00,
.byte	$FC
.byte	$66
.byte	$66
.byte	$7C
...
.byte	$E6
.byte	0
! 8601  0x78, 0xcc, 0xe0, 0x7
.byte	$78
.byte	$CC
.byte	$E0
! 8601 0, 0x1c, 0xcc, 0x78, 0x00,
.byte	$70
.byte	$1C
.byte	$CC
.byte	$78
.byte	0
! 8602  0xfc, 0xb4, 0x30, 0x30, 0x30, 0x30, 0x78, 0x00,
.byte	$FC
.byte	$B4
.byte	$30
.byte	$30
...
.byte	$78
.byte	0
! 8603  0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0xfc, 0x00,
.byte	$CC
.byte	$CC
.byte	$CC
.byte	$CC
...
.byte	$FC
.byte	0
! 8604  0xcc, 0xcc, 0xcc, 0xcc, 0xcc, 0x78, 0x30, 0x00,
.byte	$CC
.byte	$CC
.byte	$CC
.byte	$CC
...
.byte	$30
.byte	0
! 8605  0xc6, 0xc6, 0xc6, 0xd6, 0xfe, 0xee, 0xc6, 0x00,
.byte	$C6
.byte	$C6
.byte	$C6
.byte	$D6
...
.byte	$C6
.byte	0
! 8606  0xc6, 0xc6, 0x6c, 0x38, 0x38, 0x6c, 0xc6, 0x00,
.byte	$C6
.byte	$C6
.byte	$6C
.byte	$38
...
.byte	$C6
.byte	0
! 8607  0xcc, 0xcc, 0xcc, 0x78, 0x30, 0x30, 0x78, 0x00,
.byte	$CC
.byte	$CC
.byte	$CC
.byte	$78
...
.byte	$78
.byte	0
! 8608  0xfe, 0xc6, 0x8c, 0x18, 0x32, 0x66, 0xfe, 0x00,
.byte	$FE
.byte	$C6
.byte	$8C
.byte	$18
...
.byte	$FE
.byte	0
! 8609  0x78, 0x60, 0x60, 0x60, 0x60, 0x60, 0x78, 0x00,
.byte	$78
.byte	$60
.byte	$60
.byte	$60
...
.byte	$78
.byte	0
! 8610  0xc0, 0x60, 0x30, 0x18, 0x0c, 0x06, 0x02, 0x00,
.byte	$C0
.byte	$60
.byte	$30
.byte	$18
...
.byte	2
.byte	0
! 8611  0x78, 0x18, 0x18, 0x18, 0x18, 0x18, 0x78, 0x00,
.byte	$78
.byte	$18
.byte	$18
.byte	$18
...
.byte	$78
.byte	0
! 8612  0x10, 0x38, 0x6c, 0xc6, 0x00, 0x00, 0x00, 0x00,
.byte	$10
.byte	$38
.byte	$6C
.byte	$C6
...
.byte	0
.byte	0
! 8613  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff,
.byte	0
.byte	0
.byte	0
.byte	0
...
.byte	0
.byte	$FF
! 8614  0x30, 0x30, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	$30
.byte	$30
.byte	$18
.byte	0
...
.byte	0
.byte	0
! 8615  0x00, 0x00, 0x78, 0x0c, 0x7c, 0xcc, 0x76, 0x00,
.byte	0
.byte	0
.byte	$78
.byte	$C
...
.byte	$76
.byte	0
! 8616  0xe0, 0x60, 0x60, 0x7c, 0x66, 0x66, 0xdc, 0x00,
.byte	$E0
.byte	$60
.byte	$60
.byte	$7C
...
.byte	$DC
.byte	0
! 8617  0x00, 0x00, 0x78, 0xcc, 0xc0, 0xcc, 0x78, 0x00,
.byte	0
.byte	0
.byte	$78
.byte	$CC
...
.byte	$78
.byte	0
! 8618  0x1c, 0x0c, 0x0c, 0x7c, 0xcc, 0xcc, 0x76, 0x00,
.byte	$1C
.byte	$C
.byte	$C
.byte	$7C
...
.byte	$76
.byte	0
! 8619  0x00, 0x00, 0x78, 0xcc, 0xfc, 0xc0, 0x78, 0x00,
.byte	0
.byte	0
.byte	$78
.byte	$CC
...
.byte	$78
.byte	0
! 8620  0x38, 0x6c, 0x60, 0xf0, 0x60, 0x60, 0xf0, 0x00,
.byte	$38
.byte	$6C
.byte	$60
.byte	$F0
...
.byte	$F0
.byte	0
! 8621  0x00, 0x00, 0x76, 0xcc, 0xcc, 0x7c, 0x0c, 0xf8,
.byte	0
.byte	0
.byte	$76
.byte	$CC
...
.byte	$C
.byte	$F8
! 8622  0xe0, 0x60, 0x6c, 0x76, 0x66, 0x66, 0xe6, 0x00,
.byte	$E0
.byte	$60
.byte	$6C
.byte	$76
...
.byte	$E6
.byte	0
! 8623  0x30, 0x00, 0x70, 0x30, 0x30, 0x30, 0x78, 0x00,
.byte	$30
.byte	0
.byte	$70
.byte	$30
...
.byte	$78
.byte	0
! 8624  0x0c, 0x00, 0x0c, 0x0c, 0x0c, 0xcc, 0xcc, 0x78,
.byte	$C
.byte	0
.byte	$C
.byte	$C
...
.byte	$CC
.byte	$78
! 8625  0xe0, 0x60, 0x66, 0x6c, 0x78, 0x6c, 0xe6, 0x00,
.byte	$E0
.byte	$60
.byte	$66
.byte	$6C
...
.byte	$E6
.byte	0
! 8626  0x70, 0x30, 0x30, 0x30, 0x30, 0x30, 0x78, 0x00,
.byte	$70
.byte	$30
.byte	$30
.byte	$30
...
.byte	$78
.byte	0
! 8627  0x00, 0x00, 0xcc, 0xfe, 0xfe, 0xd6, 0xc6, 0x00,
.byte	0
.byte	0
.byte	$CC
.byte	$FE
...
.byte	$C6
.byte	0
! 8628  0x00, 0x00, 0xf8, 0xcc, 0xcc, 0xcc, 0xcc, 0x00,
.byte	0
.byte	0
.byte	$F8
.byte	$CC
...
.byte	$CC
.byte	0
! 8629  0x00, 0x00, 0x78, 0xcc, 0xcc, 0xcc, 0x78, 0x00,
.byte	0
.byte	0
.byte	$78
.byte	$CC
...
.byte	$78
.byte	0
! 8630  0x00, 0x00, 0xdc, 0x66, 0x66, 0x7c, 0x60, 0xf0,
.byte	0
.byte	0
.byte	$DC
.byte	$66
...
.byte	$60
.byte	$F0
! 8631  0x00, 0x00, 0x76, 0xcc, 0xcc, 0x7c, 0x0c, 0x1e,
.byte	0
.byte	0
.byte	$76
.byte	$CC
...
.byte	$C
.byte	$1E
! 8632  0x00, 0x00, 0xdc, 0x76, 0x66, 0x60, 0xf0, 0x00,
.byte	0
.byte	0
.byte	$DC
.byte	$76
...
.byte	$F0
.byte	0
! 8633  0x00, 0x00, 0x7c, 0xc0, 0x78, 0x0c, 0xf8, 0x00,
.byte	0
.byte	0
.byte	$7C
.byte	$C0
...
.byte	$F8
.byte	0
! 8634  0x10, 0x30, 0x7c, 0x30, 0x30, 0x34, 0x18, 0x00,
.byte	$10
.byte	$30
.byte	$7C
.byte	$30
...
.byte	$18
.byte	0
! 8635  0x00, 0x00, 0xcc, 0xcc, 0xcc, 0xcc, 0x76, 0x00,
.byte	0
.byte	0
.byte	$CC
.byte	$CC
...
.byte	$76
.byte	0
! 8636  0x00, 0x00, 0xcc, 0xcc, 0xcc, 0x78, 0x30, 0x00,
.byte	0
.byte	0
.byte	$CC
.byte	$CC
...
.byte	$30
.byte	0
! 8637  0x00, 0x00, 0xc6, 0xd6, 0xfe, 0xfe, 0x6c, 0x00,
.byte	0
.byte	0
.byte	$C6
.byte	$D6
...
.byte	$6C
.byte	0
! 8638  0x00, 0x00, 0xc6, 0x6c, 0x38, 0x6c, 0xc6, 0x00,
.byte	0
.byte	0
.byte	$C6
.byte	$6C
...
.byte	$C6
.byte	0
! 8639  0x00, 0x00, 0xcc, 0xcc, 0xcc, 0x7c, 0x0c, 0xf8,
.byte	0
.byte	0
.byte	$CC
.byte	$CC
...
.byte	$C
.byte	$F8
! 8640  0x00, 0x00, 0xfc, 0x98, 0x30, 0x64, 0xfc, 0x00,
.byte	0
.byte	0
.byte	$FC
.byte	$98
...
.byte	$FC
.byte	0
! 8641  0x1c, 0x30, 0x30, 0xe0, 0x30, 0x30, 0x1c, 0x00,
.byte	$1C
.byte	$30
.byte	$30
.byte	$E0
...
.byte	$1C
.byte	0
! 8642  0x18, 0x18, 0x18, 0x00, 0x18, 0x18, 0x18, 0x00,
.byte	$18
.byte	$18
.byte	$18
.byte	0
...
! 8643  0xe0, 0x30,
.byte	$E0
! 8643  0x30, 0x1c, 0x30, 0x30, 0xe0, 0x00,
.byte	$30
.byte	$30
.byte	$1C
.byte	$30
...
.byte	$E0
.byte	0
! 8644  0x76, 0xdc, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
.byte	$76
.byte	$DC
.byte	0
.byte	0
...
.byte	0
.byte	0
! 8645  0x00, 0x10, 0x38, 0x6c, 0xc6, 0xc6, 0xfe, 0x00,
.byte	0
.byte	$10
.byte	$38
.byte	$6C
...
.106:
.87C:
.ascii	"apmbios pcibios pnpbios eltorito rombios"
.ascii	"32 "
.byte	$A,$A
.byte	0
.105:
