  ...
  if (BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].cache.u.segment.d_b)
    ...
    if (protected_mode()) {
      if ( !can_push(&BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].cache, temp_ESP, 32) ) {
        BX_CPU_THIS_PTR panic("PUSHAD(): stack doesn't have enough room!\n");
      ...
      if (temp_ESP < 32)
        BX_CPU_THIS_PTR panic("pushad: eSP < 32\n");
      }
...
BX_CPU_C::POPAD32(BxInstruction_t *i)
{
  ...
  BX_CPU_THIS_PTR panic("POPAD not supported on an 8086\n");
    ...
    if (protected_mode()) {
      if ( !can_pop(32) ) {
        BX_CPU_THIS_PTR panic("pop_ad: not enough bytes on stack\n");
        exception(BX_SS_EXCEPTION, 0, 0);
        return;
        }
      }
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("ENTER_IwIb: not supported by 8086!\n");
#else
  Bit32u frame_ptr32;
  Bit16u frame_ptr16;
  Bit8u level;
  ...
  level %= 32;
/* ??? */
if (level) BX_CPU_THIS_PTR panic("enter(): level > 0\n");
//if (BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].cache.u.segment.d_b && i->os_32==0) {
//  BX_CPU_THIS_PTR info("enter(): stacksize!=opsize: I'm unsure of the code for this\n");
//  BX_CPU_THIS_PTR panic("         The Intel manuals are a mess on this one!\n");
//  }
  ...
  if ( protected_mode() ) {
    ...
    if ( !can_push(&BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].cache, temp_ESP, bytes_to_push) ) {
      BX_CPU_THIS_PTR panic("ENTER: not enough room on stack!\n");
      exception(BX_SS_EXCEPTION, 0, 0);
      }
    }
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("LEAVE: not supported by 8086!\n");
#else
  Bit32u temp_EBP;
  ...
  if ( protected_mode() ) {
    if (BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].cache.u.segment.c_ed) { /* expand up */
      if (temp_EBP <= BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].cache.u.segment.limit_scaled) {
        BX_CPU_THIS_PTR panic("LEAVE: BP > BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].limit\n");
        exception(BX_SS_EXCEPTION, 0, 0);
        return;
        }
      }
