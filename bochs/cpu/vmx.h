...
enum VMX_vmexit_reason {
   ...
   VMX_VMEXIT_INSTRUCTION_TIMEOUT = 75,
...
enum VMX_vmabort_code {
   VMABORT_SAVING_GUEST_MSRS_FAILURE = 1,
   VMABORT_HOST_PDPTR_CORRUPTED = 2,
   VMABORT_VMEXIT_VMCS_CORRUPTED = 3,
   VMABORT_LOADING_HOST_MSRS = 4,
   VMABORT_VMEXIT_MACHINE_CHECK_ERROR = 5,
   VMABORT_VMEXIT_TO_32BIT_HOST_FROM_64BIT_GUEST = 6
};
...
#define VMCS_64BIT_CONTROL_SEAM_SHARED_EPT_POINTER            0x0000203C
#define VMCS_64BIT_CONTROL_SEAM_SHARED_EPT_POINTER_HI         0x0000203D
...
#define VMCS_32BIT_CONTROL_INSTRUCTION_TIMEOUT_CTRL        0x00004024
#define VMCS_32BIT_CONTROL_SEAM_GUEST_KEYID                0x00004026 /* SEAM (not implemented) */
...
#define VMCS_DATA_OFFSET                         (0x10)           /* 16-bytes allocated for above fields */

// 16-bit encodings: 4 types, 32 encodings (0x20), 4 bytes each field =>  512 bytes
// 64-bit encodings: 4 types, 80 encodings (0x50), 4 bytes each field => 1280 bytes
// 32-bit encodings: 4 types, 48 encodings (0x30), 4 bytes each field =>  768 bytes
// Natural-width encodings: 4 types, 48 encodings (0x30), 4 bytes each field => 768 bytes
...
const Bit32u VMX_MSR_VMX_PINBASED_CTRLS_LO = 0x00000016;
...
// Allowed 0-settings (must be '1 bits)
const Bit32u VMX_MSR_VMX_VMEXIT_CTRLS_LO = 0x00036DFF;
// Allowed 1-settings
...
// Allowed 0-settings (must be '1 bits)
const Bit32u VMX_MSR_VMX_VMENTRY_CTRLS_LO = 0x000011FF;
// Allowed 1-settings
...
// IA32_VMX_MSR_PROCBASED_CTRLS3 MSR (0x492)
// -----------------------------
