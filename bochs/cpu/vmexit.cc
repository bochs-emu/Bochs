  ...
  if (! VMEXIT(VMX_VM_EXEC_CTRL2_MSR_BITMAPS)) vmexit = 1;
    ...
    if (msr >= BX_VMX_HI_MSR_START) {
       if (msr > BX_VMX_HI_MSR_END) vmexit = 1;
         ...
         BX_NOTIFY_PHY_MEMORY_ACCESS(pAddr, 1, BX_MEMTYPE_INVALID, BX_READ, BX_MSR_BITMAP_ACCESS, &field);
         if (field & (1 << (msr & 7)))
            vmexit = 1;
       }
    }
  ...
  if (VMEXIT(VMX_VM_EXEC_CTRL2_IO_BITMAPS)) {
     ...
     if ((port + len) > 0x10000) vmexit = 1;
        ...
        if ((port & 0x7fff) + len > 0x8000) {
          ...
          BX_NOTIFY_PHY_MEMORY_ACCESS(pAddr, 1, BX_MEMTYPE_INVALID, BX_READ, BX_IO_BITMAP_ACCESS, &bitmap[0]);

          pAddr = BX_CPU_THIS_PTR vmcs.io_bitmap_addr[1];
          access_read_physical(pAddr, 1, &bitmap[1]);
          BX_NOTIFY_PHY_MEMORY_ACCESS(pAddr, 1, BX_MEMTYPE_INVALID, BX_READ, BX_IO_BITMAP_ACCESS, &bitmap[1]);
        }
        else {
...
bx_bool BX_CPP_AttrRegparmN(1) BX_CPU_C::Vmexit_Vmread(bxInstruction_c *i)
{
  ...
  BX_NOTIFY_PHY_MEMORY_ACCESS(pAddr, 1, BX_MEMTYPE_INVALID, BX_READ, BX_VMREAD_BITMAP_ACCESS, &bitmap);
...
bx_bool BX_CPP_AttrRegparmN(1) BX_CPU_C::Vmexit_Vmwrite(bxInstruction_c *i)
{
  ...
  access_read_physical(pAddr, 1, &bitmap);
  BX_NOTIFY_PHY_MEMORY_ACCESS(pAddr, 1, BX_MEMTYPE_INVALID, BX_READ, BX_VMWRITE_BITMAP_ACCESS, &bitmap);
...
void BX_CPU_C::Virtualization_Exception(Bit64u qualification, Bit64u guest_physical, Bit64u guest_linear)
{
  ...
  BX_NOTIFY_PHY_MEMORY_ACCESS(vm->ve_info_addr + 4, 4, BX_MEMTYPE_INVALID, BX_READ, 0, (Bit8u*)(&magic));
  if (magic != 0) return;
  ...
  BX_NOTIFY_PHY_MEMORY_ACCESS(vm->ve_info_addr, 4, BX_MEMTYPE_INVALID, BX_WRITE, 0, (Bit8u*)(&ve_info.reason));

  access_write_physical(vm->ve_info_addr + 4, 4, &ve_info.magic);
  BX_NOTIFY_PHY_MEMORY_ACCESS(vm->ve_info_addr + 4, 4, BX_MEMTYPE_INVALID, BX_WRITE, 0, (Bit8u*)(&ve_info.magic));

  access_write_physical(vm->ve_info_addr + 8, 8, &ve_info.qualification);
  BX_NOTIFY_PHY_MEMORY_ACCESS(vm->ve_info_addr + 8, 8, BX_MEMTYPE_INVALID, BX_WRITE, 0, (Bit8u*)(&ve_info.qualification));

  access_write_physical(vm->ve_info_addr + 16, 8, &ve_info.guest_linear_addr);
  BX_NOTIFY_PHY_MEMORY_ACCESS(vm->ve_info_addr + 16, 8, BX_MEMTYPE_INVALID, BX_WRITE, 0, (Bit8u*)(&ve_info.guest_linear_addr));

  access_write_physical(vm->ve_info_addr + 24, 8, &ve_info.guest_physical_addr);
  BX_NOTIFY_PHY_MEMORY_ACCESS(vm->ve_info_addr + 24, 8, BX_MEMTYPE_INVALID, BX_WRITE, 0, (Bit8u*)(&ve_info.guest_physical_addr));

  access_write_physical(vm->ve_info_addr + 32, 8, &ve_info.eptp_index);
  BX_NOTIFY_PHY_MEMORY_ACCESS(vm->ve_info_addr + 32, 8, BX_MEMTYPE_INVALID, BX_WRITE, 0, (Bit8u*)(&ve_info.eptp_index));
