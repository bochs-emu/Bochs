  ...
  if (protected_mode()) {
    if (seg == &BX_CPU_THIS_PTR sregs[BX_SREG_SS]) {
      ...
      if ((new_value & 0xfffc) == 0) { /* null selector */
        BX_CPU_THIS_PTR panic("load_seg_reg: SS: new_value == 0\n");
      ...
      if (ti == 0) { /* GDT */
        if ((index*8 + 7) > BX_CPU_THIS_PTR gdtr.limit) {
          BX_CPU_THIS_PTR panic("load_seg_reg: GDT: %s: index(%04x) > limit(%06x)\n",
        ...
        if (BX_CPU_THIS_PTR ldtr.cache.valid==0) { /* ??? */
          BX_CPU_THIS_PTR info("load_seg_reg: LDT invalid\n");
          exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
          return;
          }
        if ((index*8 + 7) > BX_CPU_THIS_PTR ldtr.cache.u.ldt.limit) {
          BX_CPU_THIS_PTR info("load_seg_reg ss: LDT: index > limit\n");
          exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
          return;
          }
      ...
      if (rpl != CPL) {
        BX_CPU_THIS_PTR info("load_seg_reg(): rpl != CPL\n");
        exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
        return;
        }
      ...
      if (descriptor.valid==0) {
        BX_CPU_THIS_PTR info("load_seg_reg(): valid bit cleared\n");
        exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
        return;
        }
      ...
      if ( (descriptor.segment==0) ||
        ...
        BX_CPU_THIS_PTR info("load_seg_reg(): not writable data segment\n");
        exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
        }

      /* DPL in the AR byte must equal CPL else #GP(selector) */
      if (descriptor.dpl != CPL) {
        BX_CPU_THIS_PTR info("load_seg_reg(): dpl != CPL\n");
        exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
        }

      /* segment must be marked PRESENT else #SS(selector) */
      if (descriptor.p == 0) {
        BX_CPU_THIS_PTR info("load_seg_reg(): not present\n");
      ...
      if (ti == 0) { /* GDT */
        if ((index*8 + 7) > BX_CPU_THIS_PTR gdtr.limit) {
          BX_CPU_THIS_PTR info("load_seg_reg: GDT: %s: index(%04x) > limit(%06x)\n",
            BX_CPU_THIS_PTR strseg(seg), (unsigned) index, (unsigned) BX_CPU_THIS_PTR gdtr.limit);
          exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
          return;
          }
        ...
        if ((index*8 + 7) > BX_CPU_THIS_PTR ldtr.cache.u.ldt.limit) {
          BX_CPU_THIS_PTR info("load_seg_reg ds,es: LDT: index > limit\n");
          exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
          return;
          }
        access_linear(BX_CPU_THIS_PTR ldtr.cache.u.ldt.base + index*8,     4, 0,
      ...
      if ( descriptor.segment==0 ||
           (descriptor.u.segment.executable==1 &&
            descriptor.u.segment.r_w==0) ) {
        BX_CPU_THIS_PTR info("load_seg_reg(): not data or readable code\n");
        exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
        return;
        }
      ...
      if ( descriptor.u.segment.executable==0 ||
        ...
        if ((rpl > descriptor.dpl) || (CPL > descriptor.dpl)) {
          BX_CPU_THIS_PTR info("load_seg_reg: RPL & CPL must be <= DPL\n");
          exception(BX_GP_EXCEPTION, new_value & 0xfffc, 0);
          return;
          }
        }
      ...
      if (descriptor.p == 0) {
        BX_CPU_THIS_PTR info("load_seg_reg: segment not present\n");
      ...
      BX_CPU_THIS_PTR panic("load_seg_reg(): invalid segment register passed!\n");
      return;
      }
    }
  ...
  if (temp->segment) { /* data/code segment descriptors */
    ...
    switch ( temp->type ) {
      ...
      case 15: // 386 trap gate
        ...
        break;
#endif
      default: BX_CPU_THIS_PTR panic("parse_descriptor(): case %d unfinished\n",
                 (unsigned) temp->type);
        temp->valid    = 0;
      }
    }
...
BX_CPU_C::load_ss(bx_selector_t *selector, bx_descriptor_t *descriptor, Bit8u cpl)
{
  ...
  if ( (BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].selector.value & 0xfffc) == 0 )
    BX_CPU_THIS_PTR panic("load_ss(): null selector passed\n");

  if ( !BX_CPU_THIS_PTR sregs[BX_SEG_REG_SS].cache.valid ) {
    BX_CPU_THIS_PTR panic("load_ss(): invalid selector/descriptor passed.\n");
    }
}
...
BX_CPU_THIS_PTR info("selector->index*8 + 7 = %u\n", (unsigned) selector->index*8 + 7);
BX_CPU_THIS_PTR info("gdtr.limit = %u\n", (unsigned) BX_CPU_THIS_PTR gdtr.limit);
      BX_CPU_THIS_PTR info("fetch_raw_descriptor: GDT: index > limit\n");
...
BX_CPU_THIS_PTR info("-----------------------------------\n");
    ...
    if (BX_CPU_THIS_PTR ldtr.cache.valid==0) {
      BX_CPU_THIS_PTR panic("fetch_raw_descriptor: LDTR.valid=0\n");
      }
    if ((selector->index*8 + 7) > BX_CPU_THIS_PTR ldtr.cache.u.ldt.limit) {
      BX_CPU_THIS_PTR panic("fetch_raw_descriptor: LDT: index > limit\n");
      exception(exception_no, selector->value & 0xfffc, 0);
      return;
      }
