...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::UCOMISH_VshWshR(bxInstruction_c *i)
{
  ...
  float_status_t status = mxcsr_to_softfloat_status_word(MXCSR);
  softfloat_status_word_rc_override(status, i);
  int rc = float16_compare_quiet(op1, op2, status);
  check_exceptionsSSE(get_exception_flags(status));
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::COMISH_VshWshR(bxInstruction_c *i)
{
  ...
  float_status_t status = mxcsr_to_softfloat_status_word(MXCSR);
  softfloat_status_word_rc_override(status, i);
  int rc = float16_compare(op1, op2, status);
  check_exceptionsSSE(get_exception_flags(status));
  BX_CPU_THIS_PTR write_eflags_fpu_compare(rc);
...
static BX_CPP_INLINE int float16_fpclass(float16 op, int selector, int daz)
{
  extern int fpclass(float_class_t op_class, int sign, int selector);

  if (daz)
    op = float16_denormal_to_zero(op);

  return fpclass(float16_class(op), float16_sign(op), selector);
}
...
{
  if (! i->opmask() || BX_SCALAR_ELEMENT_MASK(i->opmask())) {
    BX_WRITE_OPMASK(i->dst(), float16_fpclass(BX_READ_XMM_REG_LO_WORD(i->src()), i->Ib(), MXCSR.get_DAZ()));
  }
  else {
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::VSQRTSH_VshHphWshR(bxInstruction_c *i)
{
  ...
  float_status_t status = mxcsr_to_softfloat_status_word(MXCSR);
  softfloat_status_word_rc_override(status, i);
  op1.xmm16u(0) = float16_sqrt(op2, status);
  check_exceptionsSSE(get_exception_flags(status));
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::VGETEXPSH_VshHphWshR(bxInstruction_c *i)
{
  ...
  float_status_t status = mxcsr_to_softfloat_status_word(MXCSR);
  softfloat_status_word_rc_override(status, i);
  op1.xmm16u(0) = float16_getexp(op2, status);
  check_exceptionsSSE(get_exception_flags(status));
...
static BX_CPP_INLINE float16 float16_reduce(float16 a, Bit8u scale, float_status_t &status)
{
  ...
  return float16_sub(a, tmp, status);
}
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::VFCMULCSH_MASK_VshHphWshR(bxInstruction_c *i)
{
  ...
  int fma_control1 = (i->getIaOpcode() == BX_IA_V512_VFMULCSH_VshHphWsh_Kmask) ? float_muladd_negate_product : 0;
  int fma_control2 = (i->getIaOpcode() == BX_IA_V512_VFMULCSH_VshHphWsh_Kmask) ? 0 : float_muladd_negate_product;

  if (! i->opmask() || BX_SCALAR_ELEMENT_MASK(i->opmask())) {
    dst.xmm16u(0) = float16_mul(op1->xmm16u(0), op2->xmm16u(0), status);
    dst.xmm16u(1) = float16_mul(op1->xmm16u(1), op2->xmm16u(0), status);
    dst.xmm16u(0) = float16_muladd(op1->xmm16u(1), op2->xmm16u(1), dst.xmm16u(0), fma_control1, status);
    dst.xmm16u(1) = float16_muladd(op1->xmm16u(0), op2->xmm16u(1), dst.xmm16u(1), fma_control2, status);
  }
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::VFCMULCPH_MASK_VphHphWphR(bxInstruction_c *i)
{
  ...
  for (n=0, tmp_mask = mask; n < DWORD_ELEMENTS(len); n++, tmp_mask >>= 1) {
    if (tmp_mask & 0x1) {
      dst.vmm16u(2*n)   = float16_mul(op1->vmm16u(2*n),   op2->vmm16u(2*n), status);
      dst.vmm16u(2*n+1) = float16_mul(op1->vmm16u(2*n+1), op2->vmm16u(2*n), status);
    }
  }

  int fma_control1 = (i->getIaOpcode() == BX_IA_V512_VFMULCPH_VphHphWph_Kmask) ? float_muladd_negate_product : 0;
  int fma_control2 = (i->getIaOpcode() == BX_IA_V512_VFMULCPH_VphHphWph_Kmask) ? 0 : float_muladd_negate_product;

  for (n=0, tmp_mask = mask; n < DWORD_ELEMENTS(len); n++, tmp_mask >>= 1) {
    if (tmp_mask & 0x1) {
      dst.vmm16u(2*n)   = float16_muladd(op1->vmm16u(2*n+1), op2->vmm16u(2*n+1), dst.vmm16u(2*n),   fma_control1, status);
      dst.vmm16u(2*n+1) = float16_muladd(op1->vmm16u(2*n),   op2->vmm16u(2*n+1), dst.vmm16u(2*n+1), fma_control2, status);
    }
    else if (i->isZeroMasking()) {
      dst.vmm32u(n) = 0;
    }
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::VFCMADDCSH_MASK_VshHphWshR(bxInstruction_c *i)
{
  ...
  int fma_control1 = (i->getIaOpcode() == BX_IA_V512_VFMADDCSH_VshHphWsh_Kmask) ? float_muladd_negate_product : 0;
  int fma_control2 = (i->getIaOpcode() == BX_IA_V512_VFMADDCSH_VshHphWsh_Kmask) ? 0 : float_muladd_negate_product;

  if (! i->opmask() || BX_SCALAR_ELEMENT_MASK(i->opmask())) {
    dst.xmm16u(0) = float16_muladd(op1->xmm16u(0), op2->xmm16u(0), dst.xmm16u(0), 0, status);
    dst.xmm16u(1) = float16_muladd(op1->xmm16u(1), op2->xmm16u(0), dst.xmm16u(1), 0, status);
    dst.xmm16u(0) = float16_muladd(op1->xmm16u(1), op2->xmm16u(1), dst.xmm16u(0), fma_control1, status);
    dst.xmm16u(1) = float16_muladd(op1->xmm16u(0), op2->xmm16u(1), dst.xmm16u(1), fma_control2, status);
  }
  else if (i->isZeroMasking()) {
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::VFCMADDCPH_MASK_VphHphWphR(bxInstruction_c *i)
{
  ...
  for (n=0, tmp_mask = mask; n < DWORD_ELEMENTS(len); n++, tmp_mask >>= 1) {
    if (tmp_mask & 0x1) {
      dst.vmm16u(2*n)   = float16_muladd(op1->vmm16u(2*n),   op2->vmm16u(2*n), dst.vmm16u(2*n),   0, status);
      dst.vmm16u(2*n+1) = float16_muladd(op1->vmm16u(2*n+1), op2->vmm16u(2*n), dst.vmm16u(2*n+1), 0, status);
    }
  }

  int fma_control1 = (i->getIaOpcode() == BX_IA_V512_VFMADDCPH_VphHphWph_Kmask) ? float_muladd_negate_product : 0;
  int fma_control2 = (i->getIaOpcode() == BX_IA_V512_VFMADDCPH_VphHphWph_Kmask) ? 0 : float_muladd_negate_product;
