...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("ARPL_EwRw: not supported on 8086!\n");
#else /* 286+ */
...
BX_CPU_C::LAR_GvEw(BxInstruction_t *i)
{
  ...
  if (v8086_mode()) BX_CPU_THIS_PTR panic("protect_ctrl: v8086 mode unsupported\n");

  if (real_mode()) {
    BX_CPU_THIS_PTR panic("LAR_GvEw: not recognized in real mode\n");
    UndefinedOpcode(i);
    return;
    }
  ...
  if (descriptor.valid==0) {
    set_ZF(0);
    //BX_CPU_THIS_PTR info("lar(): descriptor valid bit cleared\n");
    return;
    }
  ...
  if ( descriptor.segment ) { /* normal segment */
    ...
    switch ( descriptor.type ) {
      ...
      default: /* rest not accepted types to LAR */
        set_ZF(0);
        BX_CPU_THIS_PTR info("lar(): not accepted type\n");
        return;
        break;
      }
...
BX_CPU_C::LSL_GvEw(BxInstruction_t *i)
{
  ...
  if (real_mode()) {
    BX_CPU_THIS_PTR panic("LSL_GvEw: not recognized in real mode\n");
    UndefinedOpcode(i);
    return;
    }
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("SLDT_Ew: not supported on 8086!\n");
#else
  if (v8086_mode()) BX_CPU_THIS_PTR panic("protect_ctrl: v8086 mode unsupported\n");

  if (real_mode()) {
    /* not recognized in real address mode */
    BX_CPU_THIS_PTR panic("SLDT_Ew: encountered in real mode.\n");
    UndefinedOpcode(i);
    }
  else {
    Bit16u val16;
...
BX_CPU_C::STR_Ew(BxInstruction_t *i)
{
  ...
  if (real_mode()) {
    // not recognized in real address mode
    BX_CPU_THIS_PTR panic("STR_Ew: encountered in real mode.\n");
    UndefinedOpcode(i);
    }
  else {
    Bit16u val16;
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("LLDT_Ew: not supported on 8086!\n");
#else
  if (v8086_mode()) BX_CPU_THIS_PTR panic("protect_ctrl: v8086 mode unsupported\n");
  ...
  if (real_mode()) {
    BX_CPU_THIS_PTR panic("lldt: not recognized in real mode\n");
    UndefinedOpcode(i);
    return;
    }
  else { /* protected mode */
    ...
    if (CPL != 0) {
      BX_CPU_THIS_PTR panic("LLDT: CPL != 0\n");
    ...
    if ( (descriptor.valid==0) ||
        descriptor.segment  ||
        (descriptor.type!=2) ) {
      BX_CPU_THIS_PTR info("lldt: doesn't point to an LDT descriptor!\n");
      exception(BX_GP_EXCEPTION, raw_selector & 0xfffc, 0);
      }

    /* #NP(selector) if LDT descriptor is not present */
    if (descriptor.p==0) {
      BX_CPU_THIS_PTR info("lldt: LDT descriptor not present!\n");
      exception(BX_NP_EXCEPTION, raw_selector & 0xfffc, 0);
      }

    if (descriptor.u.ldt.limit < 7) {
      BX_CPU_THIS_PTR info("lldt: ldtr.limit < 7\n");
      }
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("LTR_Ew: not supported on 8086!\n");
#else
  if (v8086_mode()) BX_CPU_THIS_PTR panic("protect_ctrl: v8086 mode unsupported\n");
  ...
  if (protected_mode()) {
    ...
    /* #GP(0) if the current privilege level is not 0 */
    if (CPL != 0) {
      BX_CPU_THIS_PTR panic("LTR: CPL != 0\n");
      exception(BX_GP_EXCEPTION, 0, 0);
      return;
      }
    ...
    /* if selector is NULL, invalidate and done */
    if ((raw_selector & 0xfffc) == 0) {
      BX_CPU_THIS_PTR panic("ltr: loading with NULL selector!\n");
    ...
    if (selector.ti) {
      BX_CPU_THIS_PTR panic("ltr: selector.ti != 0\n");
      return;
      }
    ...
    if ( (descriptor.valid==0) || descriptor.segment  ||
      ...
      BX_CPU_THIS_PTR panic("ltr: doesn't point to an available TSS descriptor!\n");
    ...
    BX_CPU_THIS_PTR panic("ltr_ew: not recognized in real-mode!\n");
    UndefinedOpcode(i);
    return;
    }
#endif
...
BX_CPU_C::VERR_Ew(BxInstruction_t *i)
{
  ...
  if (real_mode()) {
    BX_CPU_THIS_PTR panic("VERR_Ew: not recognized in real mode\n");
    UndefinedOpcode(i);
    return;
    }
  ...
  if ( (raw_selector & 0xfffc) == 0 ) {
    set_ZF(0);
    BX_CPU_THIS_PTR info("VERR: null selector\n");
    return;
    }
  ...
  if ( !fetch_raw_descriptor2(&selector, &dword1, &dword2) ) {
    /* not within descriptor table */
    set_ZF(0);
    BX_CPU_THIS_PTR info("VERR: not in table\n");
    return;
    }
  ...
  if ( descriptor.segment==0 ) { /* system or gate descriptor */
    ...
    BX_CPU_THIS_PTR info("VERR: system descriptor\n");
    return;
    }

  if ( descriptor.valid==0 ) {
    set_ZF(0);
    BX_CPU_THIS_PTR info("VERR: valid bit cleared\n");
    return;
    }
  ...
  if ( descriptor.u.segment.executable ) { /* code segment */
    ...
    if ( descriptor.u.segment.c_ed &&
      ...
      set_ZF(1); /* accessible */
      BX_CPU_THIS_PTR info("VERR: conforming code, OK\n");
      return;
      }
    if ( descriptor.u.segment.r_w==0 ) {
      set_ZF(0); /* inaccessible */
      BX_CPU_THIS_PTR info("VERR: code not readable\n");
      return;
      }
    /* readable, non-conforming code segment */
    if ( (descriptor.dpl<CPL) || (descriptor.dpl<selector.rpl) ) {
      set_ZF(0); /* inaccessible */
      BX_CPU_THIS_PTR info("VERR: non-coforming code not withing priv level\n");
      return;
      }
    set_ZF(1); /* accessible */
    BX_CPU_THIS_PTR info("VERR: code seg readable\n");
    return;
    }
  else { /* data segment */
    if ( (descriptor.dpl<CPL) || (descriptor.dpl<selector.rpl) ) {
      set_ZF(0); /* not accessible */
      BX_CPU_THIS_PTR info("VERR: data seg not withing priv level\n");
      return;
      }
    set_ZF(1); /* accessible */
    BX_CPU_THIS_PTR info("VERR: data segment OK\n");
    return;
    }
}
...
BX_CPU_C::VERW_Ew(BxInstruction_t *i)
{
  ...
  if (real_mode()) {
    BX_CPU_THIS_PTR panic("VERW_Ew: not recognized in real mode\n");
    UndefinedOpcode(i);
    return;
    }
  ...
  if ( (raw_selector & 0xfffc) == 0 ) {
    set_ZF(0);
    BX_CPU_THIS_PTR info("VERW: null selector\n");
    return;
    }
  ...
  if ( !fetch_raw_descriptor2(&selector, &dword1, &dword2) ) {
    /* not within descriptor table */
    set_ZF(0);
    BX_CPU_THIS_PTR info("VERW: not in table\n");
    return;
    }
  ...
  if ( descriptor.segment==0 || descriptor.u.segment.executable ) {
    set_ZF(0);
    BX_CPU_THIS_PTR info("VERW: system seg or code\n");
    return;
    }

  if ( descriptor.valid==0 ) {
    set_ZF(0);
    BX_CPU_THIS_PTR info("VERW: valid bit cleared\n");
    return;
    }
  ...
  if ( descriptor.u.segment.r_w ) { /* writable */
    if ( (descriptor.dpl<CPL) || (descriptor.dpl<selector.rpl) ) {
      set_ZF(0); /* not accessible */
      BX_CPU_THIS_PTR info("VERW: writable data seg not within priv level\n");
      return;
      }
    set_ZF(1); /* accessible */
    BX_CPU_THIS_PTR info("VERW: data seg writable\n");
    return;
    }

  set_ZF(0); /* not accessible */
  BX_CPU_THIS_PTR info("VERW: data seg not writable\n");
  return;
}
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("SGDT_Ms: not supported on 8086!\n");
#else
  Bit16u limit_16;
  Bit32u base_32;
  ...
  if (i->mod == 0xc0) {
    ...
    BX_CPU_THIS_PTR panic("SGDT_Ms: use of register is undefined opcode.\n");
    UndefinedOpcode(i);
    return;
    }
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("SIDT_Ms: not supported on 8086!\n");
#else
  Bit16u limit_16;
  Bit32u base_32;
  ...
  if (i->mod == 0xc0) {
    /* undefined opcode exception */
    BX_CPU_THIS_PTR panic("SIDT: use of register is undefined opcode.\n");
    UndefinedOpcode(i);
    return;
    }
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("LGDT_Ms: not supported on 8086!\n");
#else
  ...
  if (protected_mode() && (CPL!=0)) {
    BX_CPU_THIS_PTR panic("LGDT: protected mode: CPL!=0\n");
    exception(BX_GP_EXCEPTION, 0, 0);
    return;
    }
  ...
  /* op1 is a register or memory reference */
  if (i->mod == 0xc0) {
    BX_CPU_THIS_PTR panic("LGDT generating exception 6\n");
    UndefinedOpcode(i);
    return;
    }
...
{
#if BX_CPU_LEVEL < 2
  BX_CPU_THIS_PTR panic("LIDT_Ms: not supported on 8086!\n");
#else
  Bit16u limit_16;
  Bit32u base_32;
  ...
  if (protected_mode()) {
    if (CPL != 0) {
      BX_CPU_THIS_PTR panic("LIDT(): CPL(%u) != 0\n", (unsigned) CPL);
      exception(BX_GP_EXCEPTION, 0, 0);
      return;
      }
    }
  ...
  if (i->mod == 0xc0) {
    /* undefined opcode exception */
    BX_CPU_THIS_PTR panic("LIDT generating exception 6\n");
    UndefinedOpcode(i);
    return;
    }
