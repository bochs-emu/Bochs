...
float64 float16_to_float64(float16 a, float_status_t &status)
{
    int aSign = extractFloat16Sign(a);
    Bit16s aExp = extractFloat16Exp(a);
    Bit16u aSig = extractFloat16Frac(a);
    ...
    if (aExp == 0x1f) {
        if (aSig) return commonNaNToFloat64(float16ToCommonNaN(a, status));
    ...
    return packFloat64(aSign, aExp + 0x3f0, ((Bit64u)aSig) << 42);
}
...
float16 float64_to_float16(float64 a, float_status_t &status)
{
    Bit32u aSig = extractFloat16Frac(a);
    Bit16s aExp = extractFloat64Exp(a);
    int aSign = extractFloat64Sign(a);

    if (aExp == 0x7FF) {
        if (aSig) return commonNaNToFloat16(float64ToCommonNaN(a, status));
        return packFloat16(aSign, 0x1f, 0);
    }
    aSig = shift64RightJamming(aSig, 29);
    Bit32u zSig = aSig;
    if (aExp == 0) {
        if (get_denormals_are_zeros(status)) aSig = 0;
        if (zSig == 0) return packFloat16(aSign, 0, 0);
        float_raise(status, float_flag_denormal);
    }
