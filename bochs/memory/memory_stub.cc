...
Bit8u* BX_MEMORY_STUB_C::alloc_vector_aligned(Bit64u bytes, Bit64u alignment)
{
  ...
  BX_MEM_THIS actual_vector = new Bit8u[(size_t)(bytes + test_mask)];
...
Bit8u *BX_MEMORY_STUB_C::getHostMemAddr(BX_CPU_C *cpu, bx_phy_address addr, unsigned rw)
{
  ...
  // Never allow direct access in the PCI/MMIO hole. Without a handler this
  // address range must not alias into the RAM backing store.
...
void BX_MEMORY_STUB_C::writePhysicalPage(BX_CPU_C *cpu, bx_phy_address addr, unsigned len, void *data)
{
  ...
  if (linear_addr < BX_MEM_THIS len) {
    ...
    BX_DEBUG(("Write outside the limits of physical memory (0x" FMT_PHY_ADDRX " -> linear 0x" FMT_PHY_ADDRX ") (ignore)", a20addr, linear_addr));
  }
}
...
void BX_MEMORY_STUB_C::readPhysicalPage(BX_CPU_C *cpu, bx_phy_address addr, unsigned len, void *data)
{
  ...
  if (linear_addr < BX_MEM_THIS len) {
    ...
    if (len == 4) {
      * (Bit32u*) data = ReadHostDWordFromLittleEndian((Bit32u*) BX_MEM_THIS get_vector(linear_addr));
      return;
    }
    if (len == 2) {
      * (Bit16u*) data = ReadHostWordFromLittleEndian((Bit16u*) BX_MEM_THIS get_vector(linear_addr));
      return;
    }
    if (len == 1) {
      * (Bit8u *) data = * (BX_MEM_THIS get_vector(linear_addr));
      return;
    }
    ...
    while(1) {
      ...
      if ((len & 7) == 0) {
        *((Bit64u*)data_ptr) = ReadHostQWordFromLittleEndian((Bit64u*) BX_MEM_THIS get_vector(linear_addr));
        len -= 8;
        linear_addr += 8;
#ifdef BX_LITTLE_ENDIAN
        data_ptr += 8;
        ...
        *data_ptr = *(BX_MEM_THIS get_vector(linear_addr));
        if (len == 1) return;
        len--;
        linear_addr++;
#ifdef BX_LITTLE_ENDIAN
